...
* AK/SK authentication mode (recommended)
   ...
   For more information, see [Rules for generating a signature by using AK/SK](../600.api-reference/300.1.ak-sk-sign-roles.md).
...
You can use the cURL tool to call APIs. In the following example, the user admin:hello calls the Query tenants API of an OCP application with the IP address xxx.xxx.xxx.xxx, the port information 8080. Where, `YWRtaW46cm9vdA==` is the Base64 encoding of `admin:hello`.
