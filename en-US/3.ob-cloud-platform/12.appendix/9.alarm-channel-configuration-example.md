...
* Script file name: The system provides a default custom script sample `alarm_send_script_demo.sh`. You can modify this script or refer to [Python script example](#python-script-example) and [Bash script example](#bash-script-example) to create custom scripts. When you modify or create a script, make sure that the script file is stored in the `/home/admin` directory of each OCP.
