...
After OCP is started, it automatically uploads some software packages to OCP-Server. Log on to the OCP console and click **<UI-TERM key="ocp-v2.src.component.PackageUploadModal.Package">Package</UI-TERM>** in the left-side navigation pane. In the software package list, find the `ocp-agent-<version>-<buildNumber>.<operatingSystem>.<architecture>.rpm` package and verify its version, operating system, and architecture.
