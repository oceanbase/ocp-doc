...
* You have obtained and uploaded the `ocp-all-in-one-4.3.4-YYYYMMDDhhmmss.el7.x86_64.tar.gz` software package to a directory on the OCP host, and decompressed the package by running the following command. For more information, see [Prepare for the installation](400.installation-preparation.md).

    ```bash
    tar zxf ocp-all-in-one-4.3.4-YYYYMMDDhhmmss.el7.x86_64.tar.gz
    ```
