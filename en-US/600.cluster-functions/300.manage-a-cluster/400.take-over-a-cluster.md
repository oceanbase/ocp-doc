...
* If a load type has been configured for the cluster to be taken over, you need to configure a load type for the cluster again on its **<UI-TERM key="ocp-v2.src.util.menu.SummaryInvestingComZa">Overview</UI-TERM>** page after the takeover.

## Procedure

1. Log in to the OCP console.

2. In the left-side navigation pane, select **<UI-TERM key="ocp-v2.page.Cluster.ClusterList">Clusters</UI-TERM>**. The **<UI-TERM key="ocp-v2.Cluster.Component.ClusterList.ClusterList">Clusters</UI-TERM>** page automatically appears.

3. On the **<UI-TERM key="ocp-v2.page.Cluster.ClusterList">Clusters</UI-TERM>** page, click **<UI-TERM key="ocp-v2.Cluster.Import.TakeOverTheCluster">Take over Cluster</UI-TERM>** in the upper-right corner.

4. Connect to the cluster. The following two connection modes are supported:

   * **Direct Connect**: Specify **<UI-TERM key="ocp-v2.Cluster.Import.Address">Access Address</UI-TERM>**, **<UI-TERM key="ocp-v2.Scan.ReportTable.ServerInfoTable.Port">Port</UI-TERM>**, and **root@sys Password** of the cluster, and enter the password of the `proxyro` user as needed. Then, click **<UI-TERM key="ocp-v2.Credential.Component.ImportModal.NextStep">Next</UI-TERM>**.
   ...
   * **OBProxy**: Specify **<UI-TERM key="ocp-v2.Binlog.New.AccessAddress">Access Address</UI-TERM>**, **<UI-TERM key="ocp-v2.MultiCluster.RegisterOcp.Port">Port</UI-TERM>**, **<UI-TERM key="ocp-v2.Detail.Overview.ClusterName">Cluster Name</UI-TERM>**, **<UI-TERM key="ocp-v2.Cluster.Import.ClusterIdOptional">Cluster ID (Optional)</UI-TERM>**, and **root@sys Password** of the cluster, and enter the password of the `proxyro` user as needed, which is used by OBProxy to access the OceanBase cluster. Then, click **<UI-TERM key="ocp-v2.Detail.TakeOver.Next">Next</UI-TERM>**.
...
5. After the cluster is connected, perform a pre-takeover check.
   ...
   * If the check fails, hover the pointer over the icon next to **<UI-TERM key="ocp-v2.Detail.Analyze.Failed">Failed</UI-TERM>**, modify the OBServer node configuration as prompted, and then click **<UI-TERM key="ocp-v2.Cluster.Import.ReCheck">Recheck</UI-TERM>**.

6. Click **<UI-TERM key="ocp-v2.Binlog.New.Submit">Submit</UI-TERM>**.
