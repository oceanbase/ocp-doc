...
* TopSQL Query Performance Optimization Strategy: When the SQL write pressure of MonitorDB exceeds the threshold, the system will automatically start the degradation mechanism and optimize the TopSQL record feature within a reasonable range. The core goal of this strategy is to prioritize the availability of the OAS query feature, which may result in the loss of some SQL records.
* Fixed the issue of Druid DataSource leakage under high load.
* Fixed the issue where the SQL diagnostic feature cannot display indexes for tables with quotes.
* Fixed the issue where the target organization is not fully displayed when shared resources are configured after searching for an organization on the organization list page.
...
Upgraded the basic framework and runtime environment for OCP usage and fixed some known issues.
