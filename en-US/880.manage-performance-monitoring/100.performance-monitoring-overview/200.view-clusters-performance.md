...
* **Method 1**: Log in to the OCP console and go to the cluster overview page. In the left-side navigation pane, click **<UI-TERM key="ocp-v2.Monitor.AddMonitor.PerformanceMonitoring">Performance Monitoring</UI-TERM>**.

* **Method 2**: Log in to the OCP console. In the left-side navigation pane, click **<UI-TERM key="ocp-v2.src.util.menu.PerformanceMonitoring">Performance Monitoring</UI-TERM>** to go to the **<UI-TERM key="ocp-v2.Information.Detail.CreateCollectDrawer.OceanbaseCluster">OceanBase Cluster</UI-TERM>** tab.

## Prerequisites

* To view the performance monitoring data of a cluster by using **Method 1**, make sure that you have the following permissions:

  * **<UI-TERM key="ocp-v2.Auth.Role.AddRole.ResourcePermissions">Resource Permissions</UI-TERM>**: **Cluster Read-only** permission
  * **<UI-TERM key="ocp-v2.Auth.Role.Detail.MenuPermissions">Menu Permissions</UI-TERM>**: Permission on the **<UI-TERM key="ocp-v2.Detail.Monitor.Title.PerformanceMonitoring">Performance Monitoring</UI-TERM>** menu of **<UI-TERM key="ocp-v2.page.Cluster.ClusterList">Clusters</UI-TERM>**

* To view the performance monitoring data of a cluster by using **Method 2**, make sure that you have the following permissions:
  ...
  * **<UI-TERM key="ocp-v2.Auth.Role.AddRole.MenuPermissions">Menu Permissions</UI-TERM>**: Permission on the **<UI-TERM key="ocp-v2.alarm.FormListWithExclude.ClusterWithExclude.All">All</UI-TERM>** menu of performance monitoring
...
3. The **<UI-TERM key="ocp-v2.src.constant.auth.OceanbaseCluster">OceanBase Cluster</UI-TERM>** tab appears.
...
5. On the **Database Performance** tab, view the database performance monitoring data of the cluster.
   ...
   * For more information about the monitoring metrics, see [Overview of metrics](../../1900.reference-guide/300.monitoring-indicator-reference/100.overview-of-metrics.md).
   ...
   <h4>Note</h4>
   <p>In the <b>Query response time</b> monitoring chart, the <code>99-all</code> and <code>95-all</code> metrics apply to only OceanBase Database V4.2.1.7 and versions later than V4.2.1.7 but earlier than V4.2.2.0. </p>
   </main>
   ...
   4. Drill down to view monitoring data in different dimensions: If you find an abnormal metric such as a long query response time on the **<UI-TERM key="ocp-v2.src.util.monitor.ClusterDimension">Cluster Dimension</UI-TERM>** tab, perform drill-down analysis to identify the faulty server. Click the **<UI-TERM key="ocp-v2.component.MetricChart.Item.DrillDownAnalysis">Drill-down Analysis</UI-TERM>** icon in the upper-right corner of the trend chart. The following drill-down dimensions are supported: cluster > tenant > OBServer node.
      ...
      1. In the **<UI-TERM key="ocp-v2.component.MonitorSearch.DataFiltering">Data Filtering</UI-TERM>** section of the drill-down page, specify filter conditions. If you do not specify any filter conditions, the drill-down page automatically inherits filter conditions from the upper-level page.
      ...
      3. If the metric is related to the performance of a tenant or host, you can directly go to the **<UI-TERM key="ocp-v2.Host.Detail.Surveillance">Monitoring</UI-TERM>** page of the tenant or server from the trend chart.

         * To go to the **<UI-TERM key="ocp-v2.Monitor.AddMonitor.PerformanceMonitoring">Performance Monitoring</UI-TERM>** page of a tenant, hover the pointer over the **<UI-TERM key="ocp-v2.Detail.Component.DirectToTenantList.GoToTenantPerformanceMonitoring">Go to Tenant Performance Monitoring</UI-TERM>** link in the upper-right corner of the trend chart, and click the tenant name.

         * To go to the **<UI-TERM key="ocp-v2.Host.Detail.Surveillance">Monitoring</UI-TERM>** page of a host, hover the pointer over the **Go to Host Performance Monitoring** link in the upper-right corner of the trend chart, and click the IP address of the host.

      4. You can also click the button indicated by ③ or ④ in the upper-right corner of the trend chart and select the tenants or hosts whose data needs to be displayed on the trend chart.
