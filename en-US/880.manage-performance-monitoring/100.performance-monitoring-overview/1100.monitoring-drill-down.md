...
When the queries per second (QPS) is abnormal, you can drill down the QPS metric to the tenant or OBServer dimension, and then view the value of QPS in that dimension, without bothering to switch to the **<UI-TERM key="ocp-v2.Detail.Monitor.Title.PerformanceMonitoring">Performance Monitoring</UI-TERM>** page of the tenant or the **<UI-TERM key="ocp-v2.src.hook.useMenu.Monitoring">Monitoring</UI-TERM>** tab of the host.
