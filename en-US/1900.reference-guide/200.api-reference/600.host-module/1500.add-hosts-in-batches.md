...
|-------------------|-------------|----------|----------------------------|----------------|
| hostBasicDataList | ObjectArray | Yes      | --                         | HostBasicData array, which contains basic information of the host. Details are described in **HostBasicData data structure**.    |
| sshPort           | Integer     | Yes      | 22                         | The SSH port number of the host.    |
| kind              | String      | Yes      | DEDICATED_PHYSICAL_MACHINE | The type of the host. Valid values: <li>DEDICATED_PHYSICAL_MACHINE</li><li> DEDICATED_CONTAINER  </li><li>DEDICATED_ECS </li>    |
| publishPorts      | StringArray | No       | 8080:8080,8090,8090        | The port mapping. When the kind is set to DEDICATED_CONTAINER, the port mapping between the container and the physical machine must be specified.  |
...
| ├─ contents | Array    | The TaskInstance array. Details are described in **TaskInstance data structure**. |
