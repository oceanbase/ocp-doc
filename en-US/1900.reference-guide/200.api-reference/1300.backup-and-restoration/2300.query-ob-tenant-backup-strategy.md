...
**Data structure of `customBackupRangeList`**

|  Parameter             |  Type    |  Description        |
|-------------------|----------|---------------|
|  customBackupRangeList         |  Array  |  Optional. The custom backup range. Supported OBServer versions: [4.2.5.2, 4.3.0) and [4.2.1.11, 4.2.2).   |
|  backupType      |  Enum  |  Required. The backup type. Valid values: DATA and LOG.  |
|  nodeType  |  Enum  |  Required. The backup node type. Valid values: ZONE, REGION, and IDC.  |
|  nodeList           |  Array  |  Required. The backup node collection. Nodes in the same index have the same priority, and multiple nodes in the same index are separated with commas, for example: ["zone1,zone2", "zone3"].  |
