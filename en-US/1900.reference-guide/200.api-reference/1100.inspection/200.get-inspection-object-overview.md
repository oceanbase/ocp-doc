|slug|get-inspection-object-overview|
|---|---|
...
|--------|--------|--------|----------|--------|
|  objectIds  |   Array  |  No   | 1   | The ID of the inspection object in OCP, such as a cluster ID or tenant ID. Separate multiple IDs with commas (`,`).  |
...
|  scheduleStates  |  Array  |  No  |  ACTIVE  | The inspection scheduling status. Valid values: <li>`ACTIVE`: The inspection is enabled. </li><li>`INACTIVE`: The inspection is disabled. </li><li>`EXPIRED`: The inspection has expired. </li>  |
...
|  ├─ contents |  Array    | For more information, see **Data structure of `InspectionObjectOverview`**.    |
