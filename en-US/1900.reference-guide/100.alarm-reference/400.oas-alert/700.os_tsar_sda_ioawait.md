...
* **Overview of alerts**
  ...
  * Example: obcluster=obcluster-1631964370:svr_ip=xxx.xxx.xxx.xxx:device=dm-0 OceanBase server disk I/O latency (ioawait) is excessively high
* **Alert Details**
  * Template: cluster: \${ob_cluster_name}, host: \${host}, alarm: \${alarm_name}. OceanBase disk: \${ob_device}, device \${device}, io await \${io_await_value_zh_cn} exceeds \${io_await_alarm_threshold} ms, io util \${io_util_value_zh_cn} exceeds ${io_util_alarm_threshold} %.
  * Example: Cluster: obcluster-1, Host: xxx.xxx.xxx.xxx, Alert: OceanBase server has high disk I/O await. OceanBase disk: ob_log, device dm-0, I/O await 300 ms exceeds 200 ms, I/O util 99.1% exceeds 99%.
* **Alert recovery**
  * Template: Alarm: ${alarm_name}, OBServer Disk io_await: \${io_await_value_zh_cn}, OBServer Disk io_util: \${io_util_value_zh_cn}
  * Example: Alert: OceanBase server disk I/O latency is too high. OBServer disk I/O latency: 120 ms. OBServer disk I/O utilization: 85%.
