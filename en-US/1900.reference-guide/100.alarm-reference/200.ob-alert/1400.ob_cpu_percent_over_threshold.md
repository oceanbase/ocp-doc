|slug|ob_cpu_percent_over_threshold|
|---|---|
...
| Source            | ```select /*+read_consistency(weak)*/ tenant_name, tenant_id, stat_id, value from v$sysstat, __all_tenant where stat_id IN (140005, 140006) and (con_id > 1000 or con_id = 1) and __all_tenant.tenant_id = v$sysstat.con_id;``` <blockquote>  **<UI-TERM key="ocp-v2.src.component.AddBinlogCredentialModal.Description">Note</UI-TERM>** <br>  The value of the value field is used as the value of the collected metric, and the values of other fields are used as labels.</blockquote> |
