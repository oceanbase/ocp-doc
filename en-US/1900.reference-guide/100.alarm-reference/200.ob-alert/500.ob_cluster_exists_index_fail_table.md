|slug|ob_cluster_exists_index_fail_table|
|---|---|
...
| Source            | <li> If you use OceanBase Database V4.0 or later: ```select /*+ MONITOR_AGENT READ_CONSISTENCY(WEAK) QUERY_TIMEOUT(100000000) */ count(*) as cnt from gv$table where table_type in (5) and index_status in (5, 6, 7)```</li><li>If you use OceanBase Database of a version earlier than V4.0: ```select /*+ MONITOR_AGENT QUERY_TIMEOUT(100000000) */ count(*) as cnt from CDB_INDEXES where status in ('ERROR','UNUSABLE')```</li>  |
