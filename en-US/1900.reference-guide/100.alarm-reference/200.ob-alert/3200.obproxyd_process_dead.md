...
|     Parameter     |                                                                                                                                                       Value                                                                                                                                                       |
|-------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Metric            | obproxyd_process_exists <blockquote>**<UI-TERM key="ocp-v2.page.Credential.Binlog.Description">Note</UI-TERM>** <br> The value of the metric indicates whether the obproxyd.sh process exists. The value 1 indicates that the obproxyd.sh process exists. The value 0 indicates that the obproxyd.sh process does not exist, in which case, this alert is triggered.</blockquote> |
| Source            | `ps -ef|grep -w obproxyd.sh|grep -v grep|wc -l`  <blockquote>**<UI-TERM key="ocp-v2.page.Credential.Binlog.Description">Note</UI-TERM>** <br> The metric source of this alert is special. OCP-Agent runs the preceding Linux command to verify whether the daemon of OBProxy exists.        </blockquote>                                                 |
...
* **Overview**
  ...
  * Example: obproxy_cluster_id=3:obproxy_cluster=obproxy_02-1:svr_ip=xxx.xxx.xxx.xxx. The obproxyd.sh process does not exist.

* **Details**

  * Template: OBProxy Cluster: ${obproxy_cluster}, Host: ${host}, Alert: ${alarm_name}.
  * Example: OBProxy Cluster: obproxy_02, Host: xxx.xxx.xxx.xxx, Alert: The obproxyd.sh process does not exist.

* **Alert recovery**

  * Template: Alert: ${alarm_name}, OBProxy process status: \${recover_value}
  * Example: Alert: The obproxyd.sh process does not exist, OBProxy process status: 1
...
2. Run the ssh command to log on to the OBProxy server.
   ...
   * If you cannot log on and the system prompts that the server is busy, we recommend that you restart the OBProxy.
     ...
     2. Find the faulty OBProxy server in the OBProxy servers list, and then click **<UI-TERM key="ocp-v2.Component.BinlogInstanceList.Restart">Restart</UI-TERM>** in the **<UI-TERM key="ocp-v2.Restore.SamplingStrategy.Operation">Actions</UI-TERM>** column.
