...
|     Parameter     |        Value     |
|-------------------|-----------------|
| Metric            | merge_error_flag      |
| Source            | SQL: `select /*+ MONITOR_AGENT READ_CONSISTENCY(WEAK) */ zone, name, value, time_to_usec(now()) as current from __all_zone`  |
| Collected metric  | zone_value     |
| Metric expression | max(ob_zone_stat{name="is_merge_error",@LABELS}) by (@GBLABELS)     |
