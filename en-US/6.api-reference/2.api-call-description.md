...
You can use the cURL tool to call APIs. In the following example, the user admin:hello calls the Query tenants API of an OCP application with the IP address 192.168.0.1, the port information 8080. Where, `YWRtaW46cm9vdA==` is the Base64 encoding of `admin:hello`.
