...
* Check whether the amount of business traffic is too large. For example, the amount of traffic can be large during data import. 
  ...
  * If yes, the amount of business traffic is large. 
    ...
    Apply throttling to the OceanBase cluster. For more information, see [Apply throttling to an OceanBase cluster](/en-US/4.alarm-reference/4.alarm-appendix/5.limit-the-inbound-traffic-of-the-oceanbase-cluster.md). 
...
* Check for minor compaction or major compaction exceptions. 
  ...
  Typically, minor compaction and major compaction exceptions can be fixed by restarting the OBServer. For more information, see [Restart an OBServer](/en-US/3.ob-cloud-platform/4.manage-clusters/3.basic-operations/8.manage-the-observer-cluster/4.restart-observer.md).
...
* Check for disk write errors. 
  ...
  In the case of a disk error, see [Exception handling for OceanBase cluster compaction](/en-US/4.alarm-reference/4.alarm-appendix/3.handle-oceanbase-cluster-merge-exceptions.md).
