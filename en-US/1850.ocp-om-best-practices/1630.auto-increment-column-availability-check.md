|slug|auto-increment-column-availability-check|
|---|---|
...
* **ORDER**
  ...
  * **Scenario 1: A server is restarted or breaks down.**
      ...
      In `ORDER` mode, the in-memory range of cached auto-increment values is maintained on the leader node. When the leader node is restarted or breaks down, the unused auto-increment values in the range are no longer used, and a new value range is used. As a result, value hopping occurs.
