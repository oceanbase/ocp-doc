...
* If the OceanBase cluster has an arbitration service configured, migrate the arbitration service out of the cluster and take it over to the new OCP before you take over the OBProxy. For more information, see [Migrate an arbitration service](../600.cluster-functions/1350.manage-arbitration-services/700.migrate-an-arbitration-service.md) and [Take over an arbitration service](../600.cluster-functions/1350.manage-arbitration-services/250.take-over-arbitration-serveices.md).
...
3. In the dialog box that appears, confirm the information and submit a task to move out the OBProxy cluster.
    ...
    1. If the OBProxy cluster to be moved out is associated with OceanBase Load Balancer (OBLB) or Alibaba Cloud Server Load Balancer (SLB) instances, information about these instances is displayed in the dialog box. If you want to continue to use these load balancer instances after the takeover, you can click **<UI-TERM key="ocp-v2.Monitor.Dashboard.Export">Export</UI-TERM>** to export the information. OCP will save the exported information in a local configuration file for the load balancer instances. You can import this file when you configure a takeover task.
...
2. Click the **...** icon in the upper-right corner and select **<UI-TERM key="ocp-v2.Detail.Overview.MoveOutOfTheCluster.1">Migrate Cluster</UI-TERM>**.

3. In the dialog box that appears, confirm the information and submit a task to move out the OceanBase cluster.

    1. Export credentials: After you configure an encryption key for the credential file, click **<UI-TERM key="ocp-v2.Credential.Component.ExportModal.ExportCredentials">Export Credential</UI-TERM>**. The system will export the credentials of the OceanBase cluster to be moved out. After the OceanBase cluster is taken over to the target OCP node, you can import its credentials in batches for future use.
...
1. On the **<UI-TERM key="ocp-v2.page.Cluster.ClusterList">Clusters</UI-TERM>** page, click **<UI-TERM key="ocp-v2.Cluster.Import.TakeOverTheCluster">Take over Cluster</UI-TERM>** in the upper-right corner.
...
3. After the cluster is connected, perform a pre-takeover check.
    ...
    2. If the check fails, hover the pointer over the icon next to **<UI-TERM key="ocp-v2.src.constant.multiCluster.Failed">Failed</UI-TERM>**, modify the OBServer node configuration as prompted, and then click **<UI-TERM key="ocp-v2.Cluster.Import.ReCheck">Recheck</UI-TERM>**.
...
2. Click **<UI-TERM key="ocp-v2.Detail.Overview.ConnectingTubeObproxy">Take over OBProxy</UI-TERM>** in the upper-right corner to go to the OBProxy takeover page.
