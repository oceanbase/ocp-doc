...
* Zones with replicas
  ...
  * Click the **More** icon on the right of the zone icon to view supported operations.
    ...
    * Click **Modify Resource Pool** and modify the resource pool of the zone in the dialog box that appears.
         ...
         > **Note**
         >
         > If the version of the OceanBase Database is V4.0 and later, you can only set the same number of units for all zones. For more information about how to change the number of units in all zones, see [Edit a tenant replica in a zone](2.manage-basic-tenant-operations/9.edit-a-zone.md).
