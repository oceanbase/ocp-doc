...
This topic describes frequently asked questions about monitoring metrics on the **<UI-TERM key="ocp-v2.Detail.Overview.ClusterOverview">Cluster Overview</UI-TERM>** , **<UI-TERM key="ocp-v2.Detail.Component.AddDatabaseDrawer.TenantOverview">Tenant Overview</UI-TERM>** , and **OBProxy** pages.
...
**Q3: There is a data error between the zoomed-in trend chart and the original trend chart. How do I solve this?**

A: When the number of monitoring data points is too high, the chart data may have errors due to cache reasons. We recommend that you wait a few seconds before viewing the chart in the zoomed trend view.
...
**Q3: Is the** **Used Disk Space** **displayed on the** **<UI-TERM key="ocp-v2.Detail.Overview.ClusterOverview">Cluster Overview</UI-TERM>** **page a sum of the sizes of** **multiple replicas** **or the size of a** **single replica** **?**

A: The Used Disk Space displayed on the Cluster Overview page is the sum of the sizes of multiple replicas.

**Q4: How is the** **<UI-TERM key="ocp-v2.Cluster.Component.WaterLevel.AllocatedMemory">Allocated Memory</UI-TERM>** **in** **<UI-TERM key="ocp-v2.page.Diagnosis.ResourceLevel">Resource Usage</UI-TERM>** **on the** **<UI-TERM key="ocp-v2.Detail.Overview.ClusterOverview">Cluster Overview</UI-TERM>** **page calculated** **?**
...
**Q6: Why is the CPU resource displayed as negative in a V4.x cluster?**

A: In a V4.x cluster, when CPU over-provisioning and Cgroup are enabled, if the newly created tenant consumes all the remaining resources, reducing the CPU over-provisioning quota will result in the CPU resource data being displayed as negative. This behavior is expected.
