...
A: You can scale up the tenant by adjusting the Unit specifications of the tenant replica or scale out the tenant by adjusting the number of Units under the current zone of the tenant replica on the **<UI-TERM key="ocp-v2.src.util.menu.SummaryInvestingComZa">Overview</UI-TERM>** page of the tenant.

**Q2: Does OCP determine the unit to delete when I scale in a tenant? Can I specify a unit to delete?**

A: No, only the administrator has the privilege to determine the unit to delete. If your account has the administrator privileges, you can go to the **<UI-TERM key="ocp-v2.src.util.menu.TopologicalGraph">Topology</UI-TERM>** page of the tenant and select a unit to delete.
