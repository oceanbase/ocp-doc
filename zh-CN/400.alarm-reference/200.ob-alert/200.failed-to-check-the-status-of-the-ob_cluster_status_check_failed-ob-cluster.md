...
* 如果 **告警详情** 中失败原因提示的是连接失败。
  ...
  2. 查看是否 OCP 与 OBServer 之间网络故障。
     ...
     * 若不能连接上，请根据连接失败返回的信息进行排查。
       ...
       <main id="notice" type='explain'>
        <h4>说明</h4>
        <p>OBProxy 或 OBServer 相关故障的确认，可查看如下日志：<ul><li>OCP 部署的 OBProxy 日志文件位于 <code>/home/admin/logs/obproxy/log</code>。</li><li>手工部署 OBProxy 的日志文件位于 <code>`/opt/taobao/install/obproxy-?/log</code>（`?` 的值取决于您安装的 OBProxy 版本）。</li><li>OBServer 日志文件位于 <code>/home/admin/oceanbase/log</code>。</li></ul></p>
       </main>
