...
默认配置下，执行响应时间超过 100ms 的 SQL 为 SlowSQL。您可以通过如下方法修改 SlowSQL 阈值：

1. 通过 OCP-Agent 的软件包中内置的运维工具 ocp_agentctl 修改 SlowSQL 阈值。

   其中， ocp_agentctl 存在于 OCP-Agent 目录：/home/admin/ocp_agent/bin

2. 以下以修改 SlowSQL 阈值为 500 ms 为例，执行如下代码：

```sql
/home/admin/ocp_agent/bin/ocp_agentctl config -u monagent.ob.slow.sql.threshold=500ms
```
