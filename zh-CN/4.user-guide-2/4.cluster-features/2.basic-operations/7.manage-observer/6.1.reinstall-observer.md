...
本页面为您介绍重装 OBServer 的操作。重装 OBServer 会在当前主机上删除 OBServer 并进行重新安装，帮助您快速重建有问题的 OBServer。
...
4. 在 **OBServer 列表** 部分，单击需要重装的 OBServer 操作列的 **重装** 。
...
5. 在弹出框中输入 **reinstall** 并单击 **重装** 。
    ...
    > **说明**
    >
    > * 在确认重装前，您需确认需要重装的 OBServer 信息，包括：主机 IP、所属集群、所属 zone 及 CPU 架构。
    > * 重装将清理掉此 OBServer 下所有数据，请谨慎操作。
