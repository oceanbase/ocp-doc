...
您可在集群列表部分查看 **OBProxy 集群名**、**部署模式**、**可连接 OB 集群**、**访问地址**、**访问端口**、**版本** 和 **新建时间** 。同时也可进行 **添加 OBProxy** 和 **删除** （OBProxy 集群）操作。单击 **OBProxy 集群名** 下的集群名称，即可跳转到对应的集群页面，详情参见 [创建 OBProxy 集群](../../8.obproxy/1.create-an-obproxy-cluster-2.md)、[添加 OBPRoxy](../../8.obproxy/6.add-obproxy.md)、[删除 OBProxy 集群](../../8.obproxy/11.delete-obproxy-cluster-1.md)。
