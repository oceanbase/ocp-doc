...
在 OceanBase V4.x 指定 `AUTO_INCREMENT_MODE = 'NOORDER'` 和 OceanBase V4.0 以下版本创建的包含自增列的表，都是 `NOORDER` 模式的自增表。OceanBase V4.x 新增了 `ORDER` 模式的自增列，并且为建表的默认模式，更好地兼容了 MySQL 行为。
