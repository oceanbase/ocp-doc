...
* 删除租户副本前，OceanBase 集群、租户必须为 **<UI-TERM key="ocp-v2.Detail.Statistics.utils.Running">运行中</UI-TERM>** 状态。
* 删除租户副本前，必须保证租户剩余副本满足多数派，即 F/L 副本的数量大于删除前 F/L 副本总数的一半。
* OceanBase 集群删除 Zone 前，集群必须处于 **<UI-TERM key="ocp-v2.src.constant.tenant.Running">运行中</UI-TERM>** 状态。
