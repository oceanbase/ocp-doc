...
* 缩容 OBServer 时，集群需为 **<UI-TERM key="ocp-v2.Detail.Log.TaskGraph.Running">运行中</UI-TERM>** 状态；缩容租户时，集群与租户需为 **<UI-TERM key="ocp-v2.src.constant.compute.Running">运行中</UI-TERM>** 状态。
...
* 调小租户 Unit 规格。
    ...
    1. 进入待缩容租户的 **<UI-TERM key="ocp-v2.Detail.Overview.Overview">概览</UI-TERM>** 页面。

    2. 在 **<UI-TERM key="ocp-v2.Tenant.Detail.CopyDetails">副本详情</UI-TERM>** 区域，单击操作列的 **<UI-TERM key="ocp-v2.Alarm.Subscriptions.Edit">编辑</UI-TERM>**，调小 Unit 规格。
...
* 减少租户 Unit 数量。
    ...
    1. 进入待缩容租户的 **<UI-TERM key="ocp-v2.src.hook.useMenu.Overview">概览</UI-TERM>** 页面。

    2. 在 **<UI-TERM key="ocp-v2.Tenant.Detail.CopyDetails">副本详情</UI-TERM>** 区域，单击 **<UI-TERM key="ocp-v2.Detail.Overview.ModifyUnit">修改 Unit</UI-TERM>**，减少 Unit 数量。
...
1. 进入待缩容集群的 **<UI-TERM key="ocp-v2.src.hook.useMenu.Overview">概览</UI-TERM>** 页面。

2. 选择需要删除的 OBServer 节点，单击 **<UI-TERM key="ocp-v2.Tenant.Detail.BatchDelete">批量删除</UI-TERM>**，生成任务。
...
3. 等待删除 OBServer 节点的任务完成。
    ...
    任务完成后，可以在集群 **<UI-TERM key="ocp-v2.page.Overview.Overview">概览</UI-TERM>** 页面查看部署模式，并关注集群中 OBServer 节点数量是否减少。

    OBServer 节点数量减少后，租户资源与数据会在 OBServer 节点之间自动均衡，可以在集群 **<UI-TERM key="ocp-v2.src.util.menu.ResourceManagement">资源管理</UI-TERM>** 页面中 [查看 Unit 分布](../600.cluster-functions/1000.manage-cluster-resource/100.view-the-unit-distribution.md)。
