...
针对部分客户对组织权限、组织资源隔离的需求，OCP V4.3.4 对权限模块进行了重构，对组织、权限的管理更加全面。若客户有多用户、多组织的需求，可参考本节内容进行操作。
...
角色是一组权限的集合，角色需要被分配给用户才能实际生效。被分配了角色的各用户只能在自己的授权范围内进行操作，避免非专业人员的误操作及信息泄露，从而实现信息安全和传统安全。角色是属于组织的，组织之前角色隔离。
