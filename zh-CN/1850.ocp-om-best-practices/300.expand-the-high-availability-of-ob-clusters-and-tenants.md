...
* 添加 Zone 前，OceanBase 集群必须为 **<UI-TERM key="ocp-v2.Detail.Log.TaskGraph.Running">运行中</UI-TERM>** 状态。
* OCP 中存在可部署 OBServer 的主机。
* 添加租户副本前，租户必须为 **<UI-TERM key="ocp-v2.page.Information.Running">运行中</UI-TERM>** 状态。
...
**<UI-TERM key="ocp-v2.Tenant.Detail.NewCopy">新增副本</UI-TERM>** 为异步任务，触发任务后由 RS 后台调度逐步完成，如果数据量较大，数据复制需要较长时间。建议参考如下命令定期检查数据复制状态：
