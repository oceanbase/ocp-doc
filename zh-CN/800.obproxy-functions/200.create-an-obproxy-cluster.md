...
6. 为 OBProxy 集群配置启动参数。
   ...
   * 也可通过如图 ② 处，单击 **参数模板** ，选择一个参数模板，系统会将模板中的参数连同配置自动填充到此处。在未创建集群参数模板的情况下，您可单击 **新建 OBProxy 参数模板**，为 OBProxy 集群创建参数模板，详情可参考 [管理 OBProxy 参数模板](900.manage-obproxy-parameter-templates.md)。
   ...
   | 模板      | 说明    |
   |-----------|---------|
   | 4.2.1 默认参数模版    |  OBProxy V4.2.1 版本推荐使用的参数设置，供生产环境使用。   |
   | 列存副本默认参数模版 |  列存只读副本推荐使用的参数设置，供生产环境使用。 |
