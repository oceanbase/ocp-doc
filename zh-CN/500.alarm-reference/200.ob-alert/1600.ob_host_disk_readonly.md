...
**监控指标** ob_host_disk_readonly_flag 的值表示 OBServer 节点的安装目录、数据目录和日志目录所在盘是否存在只读。

当监控指标值为 1 时触发告警，表示 OBServer 节点的安装目录、数据目录和日志目录的所在盘存在只读。
...
* 如果是 OB 日志盘和数据盘只读，则 OBServer 节点无法正常运行。
