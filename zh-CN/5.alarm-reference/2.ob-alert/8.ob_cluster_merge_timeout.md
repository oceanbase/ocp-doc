...
> **说明**
>
> 合并超时并不表示合并停止，合并仍然继续进行，超时是指合并耗时超过了预期时间，需要关注原因并做对应处理。
...
1. 首先需确定合并是否卡住，通过 OCP 合并管理观察合并进度。
   ...
   * 通过上述分析确认合并是卡住，则参考 [OceanBase 集群合并异常处理](../5.appendix/3.exception-handling-for-oceanbase-cluster-compaction.md) 确认具体有问题的 OBServer 节点，如需替换可参考 [替换 OBServer 节点](../../4.user-guide-2/4.cluster-features/2.basic-operations/7.manage-observer/5.replace-observer.md) 替换掉有问题的 OBServer 节点。
