...
* 解决方式：在谓词中使用 JSON_EQUAL，例如将其嵌入 CASE 表达式中。
...
* 解决方式：请确保您正在尝试创建的全文搜索索引列的数据类型是文本类型，如 VARCHAR、TEXT 或 CHAR 等。
...
* 解决方式：优先通过 trace_id 定位目标节点日志，结合资源监控和参数调整解决。
...
* 解决方式：请不要在远程表上使用 FORALL INSERT/UPDATE/DELETE。
...
* 解决方式：请参考您的 OceanBase 数据库对应版本的《SQL 参考（Oracle 模式）》查找正确的 SQL 语句。
...
* 解决方式：减小字符串的长度，或者切换为其他支持更长字符串长度的数据类型，比如 VARCHAR2、LONG CHAR、或者 LONG RAW。
...
* 解决方式：请更正语法，仔细检查 SQL 语句，查找缺少逗号或逗号使用不正确的位置。
...
* 解决方式：检查 SQL 查询语句，找出不一致的数据类型，使用 CAST 或 CONVERT 函数将数据类型转换为相同的类型。
...
* SQLSTATE：42000
* 错误原因：表名不在 FROM 列表中。
* 解决方式：确认输入的名字是一个正确的表名，并且在 From 列表中。
