...
* 解决方式：记录错误提示信息，然后联系 OceanBase 技术支持人员协助排查。
...
* 解决方式：这个错误码对功能不影响，逻辑备份会被暂停，后续可以重新开启。
...
* 错误原因：格式文件不存在
* 解决方式：检查目标位置下的格式文件是否存在，如果不存在，需要创建该格式文件。
...
## ERROR 9098 (HY000) : normal object in object_storage(oss,etc.) can't be appended content

* OceanBase 错误码：9098
* 错误原因：在尝试向对象存储（如 OSS 等）中的普通对象追加内容时出错。
...
* 解决方式：检查恢复的配置和参数设置，确保它们与恢复目标的要求和期望一致。
...
* 错误原因：指定的租户快照不存在。
* 解决方式：确认指定的租户快照是否正确，如果快照不存在，可能需要创建新的快照或者根据实际情况进行后续处理。
...
* 错误原因：指定的租户快照已经存在。
* 解决方式：确认指定的租户快照是否已经存在，如果快照已经存在，可能需要采取其他操作或者根据实际情况进行后续处理。
...
* 错误原因：租户快照任务超时。
* 解决方式：检查快照任务的执行时间是否过长，可能需要延长超时时间或者优化任务执行流程。
...
* 错误原因：克隆租户超时。
* 解决方式：检查克隆租户的执行时间是否过长，可能需要延长超时时间或者优化克隆流程。
...
* 错误原因：租户快照锁冲突。
* 解决方式：检查快照锁冲突的具体情况，可能需要释放冲突的快照锁或者调整锁的使用方式。
...
* 解决方式：检查输入目标的校验和类型，确保使用支持的校验和类型。
...
* 错误原因：对象存储目的地无效。
* 解决方式：请检查对象存储目的地是否正确，并且确保它是一个有效的目的地。可能需要更正对象存储目的地的配置或设置。
...
* 错误原因：无法连接到对象存储目的地。
* 解决方式：请检查与对象存储目的地的连接配置，并确保网络连接正常。可能需要更正连接配置、访问对象存储密钥或修复网络连接问题。
...
* 解决方式：请检查对象存储的权限设置，并确保具有正确的 I/O 操作权限。可能需要调整对象存储权限设置或联系管理员以获取适当的权限。
...
* 解决方式：请检查指定的 `s3_region` 和端点是否正确匹配。确保使用正确的 `s3_region` 和相应的端点进行配置。
...
* 错误原因：对象存储的端点无效。
* 解决方式：请检查对象存储的端点配置是否正确。确保端点配置正确，并且可以正常访问对象存储服务。
...
* 解决方式：检查当前报错的 SQL 是否产生了超过预期的数据量（例如，预期临时文件落盘的数据量最大为 500 GB），如果符合预期，可以调整配置项 `temporary_file_max_disk_size` 到合适的值。
...
* 错误原因：出现了非预期的文件在宏块缓存目录下，可能是用户自己创建的文件。
