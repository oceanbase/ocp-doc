# backup_storage_capacity_timeout_or_interrupted 备份恢复目录容量获取超时或线程被中断
...
* **告警概述**
  ...
  * 样例：storage_url=file:///obbackup/yc225_214/inc_data/ 备份恢复目录容量获取超时或线程被中断

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，告警：\${storage_url} \${alarm_name}。获取容量超时或被中断，错误码：\${value}。
  * 样例：集群：obcluster-1，告警：file:///obbackup/yc225_214/inc_data/ 备份恢复目录容量获取超时或线程被中断。获取容量超时或被中断，错误码：1002。

* **告警恢复**

  * 模板：告警：${alarm_name}，存储容量采集错误码：\${value}
  * 样例：告警：备份恢复目录容量获取超时或线程被中断，存储容量采集错误码：-
...
1. 确认是否 OCP 与备份存储服务器之间网络故障。
   ...
   * 当存储类型为 File 时，需确认 OceanBase 集群与备份存储服务器之间是否网络异常。参考如下命令确认二者是否网络异常。
