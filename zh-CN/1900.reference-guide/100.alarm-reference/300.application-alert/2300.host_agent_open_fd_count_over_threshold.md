...
|  参数   | 值     |
|-------|----------|
| 监控指标  | host_agent_open_fd_count，该指标表示 Agent 进程的文件句柄数，当指标值大于阈值时触发告警。     |
...
|--------------------------|------|--------|------|------|
| host_agent_open_fd_count | 1000 | 0 秒  | 20 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:host=xxx.xxx.xxx.xxx:process=ocp_monagent 服务器 Agent 文件句柄数超限

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，主机：\${host}，告警：Agent 进程：\${process}，文件句柄数 \${value} 超过限制 ${alarm_threshold}。
  * 样例：集群：obcluster-1，主机：xxx.xxx.xxx.xxx，告警：Agent 进程：ocp_monagent，文件句柄数 1200 超过限制 1000。

* **告警恢复**

  * 模板：告警：${alarm_name}，服务器 Agent 文件句柄数：\${value}
  * 样例：告警：服务器 Agent 文件句柄数超限，服务器 Agent 文件句柄数：950
