...
| 参数     | 值     |
| -------- | -------- |
| 监控指标 | collect_metric_failure_rate，该指标表示 OCP-Server 的采集监控失败率，当失败率大于阈值时触发告警。 |
| 监控表达式 | ```100 * sum(rate(ocp_monitor_collect_request_errors_total{@LABELS}[60s])) by (@GBLABELS) / sum(rate(ocp_monitor_collect_request_duration_ms_count{@LABELS}[60s])) by (@GBLABELS)``` |
| 采集指标 | ocp_monitor_collect_request_errors_total、ocp_monitor_collect_request_duration_ms_count |
| 指标来源 | OCP 自监控，通过请求 `http://OCP-IP:8080/api/v2/actuator/prometheus` 查看。 |
| 采集周期 | 5 秒 |
...
| ---------- | -------- | ------- | -------- | ---------- |
| collect_metric_failure_rate | 10 | 180 秒 | 15 秒 | 5 分钟 |
...
* **告警恢复**
  ...
  * 模板：告警：${alarm_name}，采集监控失败率：\${value_shown}
  * 样例：告警：OCP-Server 采集监控失败率过高，采集监控失败率：5 %
