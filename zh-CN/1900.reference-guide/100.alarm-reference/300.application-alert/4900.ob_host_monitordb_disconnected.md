...
| 参数    | 值      |
| ------- | ------- |
| 监控指标 | monitordb_connectable，该指标表示 OCP-Agent 是否可以连接 monitordb，指标值为 `1` 时表示可以正常连接；指标值为 `0` 时表示无法连接，并触发告警。  |
| 监控表达式 | ```min(oceanbase_connectivity{target="monitordb",@LABELS}) by (@GBLABELS)```  |
| 采集指标 | oceanbase_connectivity |
| 指标来源 | 根据 OCP-Agent 配置文件中的连接串连上 monitordb，执行 `select 1` 查看是否可以连接 monitordb。 |
| 采集周期 | 5 秒 |
...
| --------- | -------- | -------- | -------- | -------- |
| monitordb_connectable | 0 | 30 秒 | 10 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=Test-1:host=xxx.xxx.xxx.xxx OCP-Agent 无法连接 monitordb

* **告警详情**

  * 模板：集群：${ob_cluster_name}，主机：${host}，告警：${alarm_name}。
  * 样例：集群：Test，主机：xxx.xxx.xxx.xxx，告警：OCP-Agent 无法连接 monitordb。

* **告警恢复**

  * 模板：告警：${alarm_name}，OCP monitordb 连接状态：\${recover_value}
  * 样例：告警：OCP-Agent 无法连接 monitordb，OCP monitordb 连接状态：0
