...
| 参数     | 值       |
|---------|----------|
| 监控指标 | monagent_boot_time_delta_seconds，该指标表示 ocp_monagent 进程的启动时间戳变化，当指标值大于阈值时表示进程停止，并触发告警。 |
...
|----------|----------------------|----------|----------|---------|
| monagent_boot_time_delta_seconds   | 10   | 0 秒  | 20 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=AdminMETA-1:host=xxx.xxx.xxx.xxx ocp_monagent 进程停止

* **告警详情**

  * 模板：集群：${ob_cluster_name}，主机：${host}，告警：${alarm_name}。
  * 样例：集群：AdminMETA，主机：xxx.xxx.xxx.xxx，告警：ocp_monagent 进程停止。

* **告警恢复**

  * 模板：告警：${alarm_name}，ocp_monagent 进程启动时间戳变化：\${recover_value}
  * 样例：告警：ocp_monagent 进程停止，ocp_monagent 进程启动时间戳变化：0 秒
