...
| 参数       | 值         |
| ---------- | ---------- |
| 监控指标   | ic_server_health，该指标表示健康检查接口反馈的结果，指标值为 `1` 时表示可连接；指标值为 `0` 时表示不可连接，并触发告警。   |
...
| ---------------- | -------- | -------- | -------- | -------- |
| ic_server_health | 0        | 120 秒     | 15 秒    | 5 分钟   |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:OCP 节点 xxx.xxx.xxx.1 无法连接 IC-Server xxx.xxx.xxx.2
