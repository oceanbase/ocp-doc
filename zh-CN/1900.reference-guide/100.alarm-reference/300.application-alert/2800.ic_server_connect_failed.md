...
* **告警概述**
  ...
  * 模板：OCP 节点 \${ocp_address} 无法连接 IC-Server \${ic_server_address}
  * 样例：OCP 节点 xxx.xxx.xxx.1 无法连接 IC-Server xxx.xxx.xxx.2
...
* **告警恢复**
  ...
  * 模板：告警：${alarm_name}，Inter Connector Server 是否健康（可正常访问）：\${recover_value}
  * 样例：告警：服务端连接失败，Inter Connector Server 是否健康（可正常访问）：1
