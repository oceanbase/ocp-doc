...
|  参数   |      值       |
|-------|-----------|
| 监控指标  | storage_capacity_monitor_error_code，该指标表示 OCP-Server 监控备份恢复存储容量时采集来的错误码，当监控指标值为 1001 时触发告警。      |
...
|-------------------------------------|------|------|-------|-------|
| storage_capacity_monitor_error_code | 1001   | 0 秒  | 60 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例: alarm_template_id=0:ob_cluster=obcluster-1:storage_url=file:///obbackup/yc225_214/ 备份恢复容量获取重试次数超限

* **告警详情**

  * 模板: 集群：\${ob_cluster_name}，告警：\${storage_url} \${alarm_name}。获取容量重试次数超限，错误码：\${value}。
  * 样例: 集群：obcluster-1，告警：file:///obbackup/yc225_214/ 备份恢复容量获取重试次数超限。获取容量重试次数超限，错误码：1001。
