...
|  参数   |   值        |
|---------|--------------|
| 监控指标  | host_agent_goroutine_count，该指标表示 Agent 进程的 Go 协程（goroutine）数量，当指标值大于阈值时触发告警。      |
...
|----------------------------|------|--------|------|------|
| host_agent_goroutine_count | 3000 | 0 秒  | 20 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:host=xxx.xxx.xxx.xxx:process=ocp_monagent 服务器 Agent 协程数超限

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，主机：\${host}，告警：Agent 进程：\${process}，协程数 \${value} 超过限制 \${alarm_threshold}。
  * 样例：集群：obcluster-1，主机：xxx.xxx.xxx.xxx，告警：Agent 进程：ocp_monagent，协程数 3500 超过限制 3000。  

* **告警恢复**

  * 模板：告警：${alarm_name}，服务器 Agent 协程数：\${value}
  * 样例：告警：服务器 Agent 协程数超限，服务器 Agent 协程数：950
