...
* **告警概述**
  ...
  * 样例: alarm_template_id=0:datasource=Monitordb:table_name=ob_hist_trans_stat_v2 分区创建失败

* **告警详情**

  * 模板: ${alarm_target} ${alarm_name} 分区（${partition_name}）创建失败，结果：${result}。
  * 样例: alarm_template_id=0:datasource=Monitordb:table_name=ob_hist_trans_stat_v2 分区创建失败 分区（P20251021）创建失败，结果：success。
...
分区守护任务相关的表详见 [分区守护表](../../../2100.appendix/300.daemon-task-managed-table.md)。