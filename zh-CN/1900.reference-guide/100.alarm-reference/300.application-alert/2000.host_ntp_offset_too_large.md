...
当 OCP 管理的服务器与时钟源之间的时钟偏移大于阈值时触发该告警。
...
|  参数   | 值       |
|-------|--------|
| 监控指标  | ntp_offset_milliseconds，该指标表示服务器与时钟源的时间差，当时间差值大于阈值或时钟同步服务不可用时触发告警。   |
...
|------------------------------|-------------|------|------|------|
| ntp_offset_milliseconds      | <li>ntp_offset_milliseconds > 100</li><li>ntp_offset_milliseconds > 50</li>    | 30 秒    | 10 秒 | 5 分钟 |

## 告警信息

|   告警触发方式   | 告警等级 | 范围  |
|------------|------|-----|
| 基于监控指标的表达式 | <li>ntp_offset_milliseconds > 100：停服</li><li>ntp_offset_milliseconds > 50：严重</li>     | 主机 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:host=xxx.xxx.xxx.xxx 服务器与时钟源偏移过大

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，主机：\${host}，告警：\${alarm_name}。服务器与时钟源偏移过大，时钟偏移（\${value_shown}）超过阈值（${alarm_threshold} 毫秒）。
  * 样例：集群：obcluster-1，主机：xxx.xxx.xxx.xxx，告警：服务器与时钟源偏移过大。服务器与时钟源偏移过大，时钟偏移（104 毫秒）超过阈值（100 毫秒）。

* **告警恢复**

  * 模板：告警：${alarm_name}，服务器与时钟源偏移量：\${value_shown}
  * 样例：告警：服务器与时钟源偏移过大，服务器与时钟源偏移量：50 毫秒
