...
# vpc_connect_failed 用户 VPC 连接失败
...
| 参数       | 值      |
| ---------- | ----------------- |
| 监控指标   | vpc_network_health，该指标表示健康检查接口反馈的结果，指标值为 `0` 时表示可连接；指标值为 `1` 时表示不可连接，并触发告警。     |
...
| ------------------ | -------- | -------- | -------- | -------- |
| vpc_network_health | 0        | 120 秒     | 15 秒    | 5 分钟   |
...
* **告警概述**
  ...
  * 模板：${alarm_target} ${alarm_name} ${vpc_description}
