...
|--------------------------------|-------------|------|------|------|
| ob_database_disk_used_gb       | 100         | 0 秒  | 20 秒 | 5 分钟 |
...
* **告警详情**
  ...
  * 模板：集群：${ob_cluster_name}，告警：${alarm_name}。回收站磁盘占用 ${value_shown} 超过 ${alarm_threshold} GiB。
  * 样例：集群：Test，告警：OceanBase 回收站磁盘占用超限。回收站磁盘占用 120 GiB 超过 100 GiB。

* **告警恢复**

  * 模板：告警：${alarm_name}，OceanBase 回收站磁盘占用：\${value_shown}
  * 样例：告警：OceanBase 回收站磁盘占用超限，OceanBase 回收站磁盘占用：90 GiB
