...
| 参数       | 值      |
| ---------- | -------- |
| 监控指标   | host_agent_process_status，该指标表示进程状态，指标值为 `1` 表示正常；指标值为 `0` 表示不正常，并触发告警。   |
...
| ------------------------- | -------- | -------- | -------- | -------- |
| host_agent_process_status | 0        | 60 秒     | 15 秒    | 5 分钟   |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:host=xxx.xxx.xxx.xxx:process=ocp_monagent Agent 服务不可用

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，主机：\${host}，告警：agent 进程不可用。进程名：\${agent_process}，进程状态：${process_status}。
  * 样例：集群：obcluster-1，主机：xxx.xxx.xxx.xxx，告警：agent 进程不可用。进程名：ocp_monagent，进程状态：不可用。

* **告警恢复**

  * 模板：告警：${alarm_name}，Agent 进程状态：\${process_status}
  * 样例：告警：Agent 服务不可用，Agent 进程状态：1
