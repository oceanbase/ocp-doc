...
|  参数   |          值           |
|-------|----------------|
| 监控指标  | ntp_service_exist、ntp_service_available，其中：<li>ntp_service_exist 指标表示时钟服务是否存在的检查结果，指标值为 `1` 时表示存在，指标值为 `0` 时表示不存在。</li><li>ntp_service_available 表示时钟服务是否可用，指标值为 `1` 时表示可用，指标值为 `0` 时表示不可用。</li>当任意一个指标值为 `0` 时触发告警。   |
| 监控表达式 | <li>ntp_service_exist：`max(process_exists{name="ntpd",@LABELS}) by (@GBLABELS) + max(process_exists{name="chronyd",@LABELS}) by (@GBLABELS)`</li><li>ntp_service_available：`min(ntp_service_available{@LABELS}) by (@GBLABELS)`</li>   |
...
|------------------------|---------|----------|----------|----------|
| <li>ntp_service_exist</li><li>ntp_service_available</li> | ntp_service_exist == 0 or ntp_service_available == 0   | 0 秒   | 10 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：svr_ip=xxx.xxx.xxx.xxx 服务器时钟同步服务不存在
