...
* * **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=cluster_A-4:tenant_name=tenant1:db_name=test:sql_id=142A49D034B27871DE8E646F3360****:recommend_operations=FLUSH_PLAN_CACHE,PLAN_BINDING SQL 巡检，SQL 计划恶化

* **告警详情**

  * 模板：在时间段 ${start_time} - ${end_time} 内，租户 ${tenant_name}，数据库 ${db_name}，发现异常 SQL ${sql_id}，计划恶化。当前执行计划 ${plan_hash_of_current_plan}，历史执行计划 ${plan_hash_of_recommend_plan}，当前平均 CPU 时间 ${sql_cpu_time_ms} ms，历史平均 CPU 时间 ${sql_baseline_cpu_time_ms} ms，建议 【刷新 Plan Cache，固定执行计划】。
  * 样例：时间段：2023-08-31T15:22:47 - 2023-08-31T15:24:17 内，租户 tenant1，数据库 test，发现异常 SQL 6CE3107B0430F8CA5AAE8B5094EC*\*\*\*，计划恶化。当前执行计划 -75141822074798*\*\**，历史执行计划 708693211292835****，当前平均 CPU 时间 278.38 ms，历史平均响应时间 0.2 ms，建议操作【刷新 Plan Cache】。
