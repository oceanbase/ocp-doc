...
# ob_cluster_sync_delay_time_too_long OceanBase 集群同步延迟时间过长
...
|     参数     |       值    |
|------------|---------------|
| 监控指标    | sync_delay_time，该指标表示 OceanBase 主备集群延迟时间，当延迟时间大于阈值时触发告警。      |
| 监控表达式  | ```max(sync_delay_time{@LABELS}) by (@GBLABELS)```    |
| 采集指标    | sync_delay_time     |
| 数据来源    | 由 OCP-Server 采集内部表 `v$OB_CLUSTER` 的 CURRENT_SCN 字段值。</br>OceanBase 集群创建 1 小时后，且主库存在普通租户而非只有系统租户时，开始采集。CURRENT_SCN 字段表示主备库一致的时间点，用当前时间减去该字段的值则表示同步延迟时间。 |
| 采集周期    | 5 秒    |
...
|-----------------|------------|------|------|------|
| sync_delay_time | 600 秒      | 0 秒  | 15 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=obcluster-1 OceanBase 集群同步延迟时间过长

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，告警：\${alarm_name} 日志传输延迟 \${value_shown} 超过 ${alarm_threshold} 秒。
  * 样例：集群：obcluster-1，告警：OceanBase 集群同步延迟时间过长，日志传输延迟 3994.293 秒超过 600.0 秒。

* **告警恢复**

  * 模板：告警：${alarm_name}，OceanBase 主备集群的传输延迟：\${value_shown}
  * 样例：告警：OceanBase 集群同步延迟时间过长，OceanBase 主备集群的传输延迟：566.0 秒
