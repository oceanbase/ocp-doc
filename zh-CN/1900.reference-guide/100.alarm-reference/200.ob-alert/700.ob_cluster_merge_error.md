...
| 指标来源  | SQL：<li>OceanBase V4.0.0 之前版本：`select zone, name, value, time_to_usec(now()) from __all_zone where name = 'merge_start_time';`</li><li>OceanBase V4.0.0 及之后版本：`select tenant_id, time_to_usec(start_time) as start_time, time_to_usec(now()) as now from CDB_OB_MAJOR_COMPACTION;`</li>  |
| 采集指标  | zone_value       |
