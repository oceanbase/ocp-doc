...
| 参数 | 值 |
| --- | --- |
| 监控指标 | arbitration_service_available，该指标表示仲裁服务进程是否可用，指标值为 `0` 表示进程可用；指标值为 `1` 表示进程不可用，并触发告警。 |
...
| --------- | -------- | -------- | -------- | -------- |
| arbitration_service_available | 0 | 120 秒 | 60 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:host=xxx.xxx.xxx.xxx:svr_port=2882 仲裁服务不可用
...
* **告警恢复**
  ...
  * 模板：告警：${alarm_name}，仲裁服务是否可用：\${recover_value}
  * 样例：告警：仲裁服务不可用，仲裁服务是否可用：0
