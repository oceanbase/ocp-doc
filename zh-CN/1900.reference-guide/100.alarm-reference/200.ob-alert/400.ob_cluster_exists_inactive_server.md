...
# ob_cluster_exists_inactive_server OceanBase 集群存在不工作 OBServer
...
|   参数    |    值        |
|---------|------------|
| 监控指标       | inactive_server_count，该指标表示 OceanBase 集群中是否存在不工作的 OBServer 节点，当指标值大于阈值时触发告警。 |
| 监控指标表达式 | ```max(ob_server_num{status="inactive",@LABELS}) by (@GBLABELS)```     |
| 采集指标       | ob_server_num       |
| 数据来源       | 采集自内部视图，采集 SQL：<li>OceanBase V4.0 以下版本：```select /*+ MONITOR_AGENT READ_CONSISTENCY(WEAK) */ group_concat(svr_ip SEPARATOR ',') as servers, status, count(1) as cnt from __all_server group by status```</li><li>OceanBase V4.0 及以上版本：```select /* MONITOR_AGENT */ group_concat(svr_ip SEPARATOR ',') as servers, status, count(1) as cnt from DBA_OB_SERVERS group by status```</li> |
| 采集周期       | 5 秒      |
...
|-----------------------|------|------|------|------|
| inactive_server_count | 0    | 30 秒  | 10 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=obcluster-1 OceanBase 集群存在不工作 OBServer

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，告警：\${alarm_name}。不工作 OBServer 数量为 \${value}，不工作 OBServer 有 ${server_ips}。
  * 样例：集群：obcluster-1，告警：OceanBase 集群存在不工作 OBServer。不工作 OBServer 数量为 2，不工作 OBServer 有 xxx.xxx.xxx.1,xxx.xxx.xxx.2。

* **告警恢复**

  * 模板：告警：\${alarm_name}，OceanBase 集群不工作 OBServer 数量：\${value}
  * 样例：告警：OceanBase 集群存在不工作 OBServer 节点，OceanBase 集群不工作 OBServer 数量：0
...
4. 检查是否 observer 进程正常但心跳不上报。
   ...
   通过 OCP 的 **<UI-TERM key="ocp-v2.Information.Detail.CollectDetails.HostList">主机列表</UI-TERM>** 进入对应 OBServer 节点，查看 **OCP Agent** 页签中的进程是否正常。
