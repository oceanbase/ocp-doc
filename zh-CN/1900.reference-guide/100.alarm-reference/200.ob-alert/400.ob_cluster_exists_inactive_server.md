# ob_cluster_exists_inactive_server OceanBase 集群存在不工作 OBServer 节点
...
|----------------------------------|------|------|------|------|
| ob_cluster_inactive_server_count | 0    | 30 秒  | 10 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：ob_cluster=obcluster-1 OceanBase 集群存在不工作 OBServer 节点

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，告警：\${alarm_name}，不工作 OBServer 节点数量为 \${value}，不工作 OBServer 节点有 ${server_ips}。
  * 样例：集群：obcluster-1，告警：OceanBase 集群存在不工作 OBServer 节点，不工作 OBServer 节点数量为 2.0，不工作 OBServer 节点有 xxx.xxx.xxx.1,xxx.xxx.xxx.2。
