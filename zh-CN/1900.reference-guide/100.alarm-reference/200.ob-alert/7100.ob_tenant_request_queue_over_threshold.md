...
| ob_tenant_request_queue_size >10 and request_queue_time > 1000 | 租户请求队列长度；租户队列请求处理耗时（单位微秒） | <ul><li>长度阈值：10</li><li>耗时阈值：1毫秒</li>

## 规则信息

|       监控指标           | 默认阈值（单位：%） | 持续时间 | 检测周期 | 消除周期 |
|--------------------------|------------|------|------|------|
| ob_tenant_request_queue_size、request_queue_time | <li>ob_tenant_request_queue_size：10 或 100</li><li>request_queue_time：1000</li>        | 300 秒  | 10 秒 | 5 分钟 |

## 告警信息

| 告警触发方式 | 告警等级 | 范围 |
| ------------ | -------- | -------- |
| 基于监控指标的表达式 | <li>ob_tenant_request_queue_size > 10 且 request_queue_time > 1000：严重</li><li>ob_tenant_request_queue_size > 100：警告 | 租户 |
