|slug|ob_host_exists_expired_trans|
|---|---|
...
OceanBase 集群中存在事务停留在提交阶段 600 秒或以上则触发该告警。

## 告警原理

下表列出了该告警监控逻辑中涉及的关键参数。

| **<UI-TERM key="ocp-performance-report.ReportTables.ParameterStatInfo.ServerParamInfoTable.Parameter">参数</UI-TERM>** | **<UI-TERM key="ocp-v2.Detail.Parameter.List.Value">值</UI-TERM>** |
| --- | --- |
| 监控指标 | `ob_host_expired_trans_count`</br>该指标表示 OBServer 节点的悬挂事务个数，当悬挂事务个数大于阈值（默认为 0）时触发告警。 |
...
| **监控指标** | **默认阈值** | **<UI-TERM key="ocp-performance-report.ReportTable.ActivityOverTime.ActivityOverTimeTable.Duration">持续时间</UI-TERM>** | **<UI-TERM key="ocp-v2.Alarm.Template.New.AlarmCycle">告警周期</UI-TERM>** | **<UI-TERM key="ocp-v2.Alarm.Template.Detail.EliminationCycle">消除周期</UI-TERM>** |
| --- | --- | --- | --- | --- |
| ob_host_expired_trans_count | 0 | 0 秒 | 60 秒  | 5 分钟 |

## 告警信息

| **告警触发方式** | **<UI-TERM key="ocp-v2.page.Overview.AlarmLevel">告警等级</UI-TERM>** | **范围** |
| --- | --- | --- |
| 基于监控指标的表达式 | 严重 | 服务器 |
