...
# ob_cannot_connected OceanBase 服务器无法连接
...
|  参数     |                      值                      |
|-----------|---------------------------------------------|
| 监控指标   | ob_connectable，该指标表示 OceanBase 服务器是否可连接，指标值为 `1` 时表示 OBServer 节点可连接；指标值为 `0` 时表示该 OBServer 节点不可连接，并触发告警。  |
| 监控表达式 | ```min(oceanbase_connectivity{@LABELS}) by (@GBLABELS)``` |
| 采集指标   | oceanbase_connectivity      |
| 指标来源   | 该告警的指标来源相对特殊，通过数据库连接池连接本机 OBServer 节点并执行 SQL 语句 `select 1;` 来检测 OBServer 节点是否可连接，然后将结果传递给采集指标。    |
| 采集周期   | 5 秒       |
...
|----------------|---------|----------|---------------|--------|
| ob_connectable | 0       | 30 秒    | 10 秒         | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=obcluster-1:host=xxx.xxx.xxx.xxx OceanBase 服务器无法连接

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，主机：\${host}，告警：\${alarm_name}。
  * 样例：集群：obcluster-1，主机：xxx.xxx.xxx.xxx，告警：OceanBase 服务器无法连接。

* **告警恢复**

  * 模板：告警：${alarm_name}，OBServer 连接状态：\${recover_value}
  * 样例：告警：OceanBase 服务器无法连接，OBServer 连接状态：1
...
2. 使用 ssh 命令登录 OBServer 节点机器看是否能正常登录。
   ...
   * 否，则 OBServer 节点机器繁忙，建议参考如下命令重启 OBServer 节点。
     ...
     也可以在 OCP 集群 **<UI-TERM key="ocp-v2.Detail.Monitor.Monitor.Overview">总览</UI-TERM>** 页的 **<UI-TERM key="ocp-v2.Component.OBServerList.ObserverList">OBServer 列表</UI-TERM>** 中发起重启任务。
