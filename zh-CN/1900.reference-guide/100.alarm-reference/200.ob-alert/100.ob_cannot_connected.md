...
|  参数     |                      值                      |
|-----------|---------------------------------------------|
| 监控指标   | ob_connectable，该指标表示 OceanBase 服务器是否可连接，指标值为 `1` 时表示 OBServer 节点不可连接；指标值为 `0` 时表示该 OBServer 节点不可连接，并触发告警。  |
...
* **告警概述**
  ...
  * 样例：ob_cluster=obcluster-1:svr_ip=xxx.xxx.xxx.xxx OceanBase 服务器无法连接
