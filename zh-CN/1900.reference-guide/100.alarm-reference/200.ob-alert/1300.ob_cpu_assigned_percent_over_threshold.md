...
# ob_cpu_assigned_percent_over_threshold OceanBase 已经分配给租户的 CPU 占比超限
...
|-----------------------|------------|------|------|------|
| cpu_assigned_percent  | 98         | 0 秒  | 20 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=obcluster-1:host=xxx.xxx.xxx.xxx OceanBase 已经分配给租户的 CPU 占比超限

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，主机：\${host}，告警：CPU 分配占比 \${value_shown} 超过 ${alarm_threshold} %。
  * 样例：集群：obcluster-1，主机：xxx.xxx.xxx.xxx，告警：CPU 分配占比 99.0 % 超过 98.0 %。

* **告警恢复**

  * 模板：告警：${alarm_name}，OceanBase 已经分配给租户的 CPU 占比：\${value_shown}
  * 样例：告警：OceanBase 已经分配给租户的 CPU 占比超限，OceanBase 已经分配给租户的 CPU 占比：97%
