|slug|ob_tenant_log_disk_usage_high|
|---|---|
...
| 参数       |  值       |
| ---------- | --------- |
| 监控指标   | ob_tenant_log_disk_usage，该指标表示租户日志盘使用率，当使用率大于阈值时触发告警。 |
...
* **告警详情**
  ...
  * 样例：集群：C1，zone：Z1，租户：T1，告警：OceanBase 租户日志磁盘使用率 96 % 超过阈值 95 %。

* **告警恢复**

  * 模板：告警：${alarm_name}，OceanBase 租户日志盘使用率：\${value_shown}
  * 样例：告警：OceanBase 租户日志磁盘使用率超限，OceanBase 租户日志盘使用率：90 %
