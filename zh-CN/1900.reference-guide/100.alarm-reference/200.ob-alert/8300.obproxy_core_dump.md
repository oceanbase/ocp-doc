...
* 告警概述
  ...
  * 样例：alarm_template_id=0:ob_cluster=TEST-1:host=192.168.1.1 OceanBase 500租户STORAGE_SHORT_TERM模块内存占比过高
...
* 告警详情
  ...
  * 样例：集群：TEST，主机：192.168.1.1，告警：OceanBase 500租户STORAGE_SHORT_TERM模块内存占比过高。ORAGE_SHORT_TERM_META_CTX_ID 内存占比 35 %（内存占用 3.5 GB）超过 30 %。
...
* 分区副本 sstable 过多，可能的原因。
  ...
  * minor_compact_trigger 设置太大，导致 mini minor 做不了
    ...
    1. Mini Minor Merge：将最新 freeze_info 区间的多个 Mini SSTable 合成一个；（V3.1 后支持 History Mini Minor 可以将历史 freeze_info 区间的多个Mini SSTable 合成一个）
