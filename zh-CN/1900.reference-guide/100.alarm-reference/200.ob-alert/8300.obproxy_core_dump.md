...
| 参数 | 值 |
| -------- | -------- |
| 监控指标 | obproxy_core_dump_seconds，该指标表示 OBProxy 最近一次 core dump 时长，当其值大于阈值时触发告警。  |
| 指标来源 | OBProxy 监控，查看 OBProxy 最近一次 core dump 时长，单位为秒 |
| 采集指标 |   |
...
| 监控表达式 | 默认阈值（单位：秒） | 检测周期 | 消除周期 |
| --------- | --------- | -------- | -------- | -------- |
| obproxy_core_dump_seconds | obproxy_core_dump_seconds < 1200 and obproxy_core_dump_seconds > 0 | 0 | 10 秒 | 5 分钟 |
...
* **告警恢复**
  ...
  * 模板：告警：${alarm_name}，距 OBProxy 最近 core dump 时长：\${value_shown}
  * 样例：告警：OBProxy server Core Dump，距 OBProxy 最近 Core Dump 时长：0 秒
