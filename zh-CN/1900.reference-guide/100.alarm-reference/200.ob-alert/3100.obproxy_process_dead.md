...
|  参数   |  值      |
|-------|-------|
| 监控指标  | `obproxy_process_exists`，该指标表示 obproxy 进程是否存在，1 表示存在，0 表示不存在，为 0 时触发告警。   |
| 指标来源  | `ps -ef\|grep -w obproxy\|grep -v grep\|wc -l`</br>该告警的指标来源相对特殊，是 OCP-Agent 执行上述 Linux 指令检查 obproxy 进程是否存在。 |
