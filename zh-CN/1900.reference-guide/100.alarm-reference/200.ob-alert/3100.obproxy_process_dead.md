...
|  参数   |  值      |
|-------|-------|
| 监控指标  | obproxy_process_exists，该指标表示 obproxy 进程是否存在，指标值为 `1` 时表示存在；指标值为 `0` 表示不存在，并触发告警。   |
| 监控表达式 | ```process_exists{name="obproxy",@LABELS}```     |
| 采集指标  | obproxy_process_exists   |
| 指标来源  | 该告警的指标来源相对特殊，是 OCP-Agent 执行如下 Linux 指令检查 obproxy 进程是否存在：<br>```ps -ef\|grep -w obproxy\|grep -v grep\|wc -l```  |
| 采集周期  | 5 秒     |
...
|------------------------|------|------|------|------|
| obproxy_process_exists | 0   | 30 秒  | 10 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:obproxy_cluster_id=3:obproxy_cluster=obproxy_02:host=xxx.xxx.xxx.xxx OBProxy 进程不存在

* **告警详情**

  * 模板：OBProxy 集群：\${obproxy_cluster}，主机：\${host}，告警：${alarm_name}。
  * 样例：OBProxy 集群：obproxy_cluster_id=3，主机：xxx.xxx.xxx.xxx，告警：OBProxy 进程不存在。

* **告警恢复**

  * 模板：告警：${alarm_name}，OBProxy 进程存活状态：\${recover_value}
  * 样例：告警：OBProxy 进程不存在，OBProxy 进程存活状态：1
...
2. 使用 ssh 命令登录 OBProxy 机器看是否能正常登录。
   ...
   * 否，则 OBProxy 机器繁忙，建议重启 OBProxy。
     ...
     2. 在 OBProxy 列表中找到故障 OBProxy 主机，并单击其对应 **<UI-TERM key="ocp-v2.Cluster.Arbitration.Operation">操作</UI-TERM>** 列的 **<UI-TERM key="ocp-v2.Component.ZoneList.Restart">重启</UI-TERM>** 。
