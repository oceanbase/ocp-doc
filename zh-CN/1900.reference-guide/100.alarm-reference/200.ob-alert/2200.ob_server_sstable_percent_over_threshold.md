|slug|ob_server_sstable_percent_over_threshold|
|---|---|
...
| 数据来源  | 采集自内部视图，采集 SQL：<ul><li>OceanBase V4.0 以下版本：```select /*+ MONITOR_AGENT READ_CONSISTENCY(WEAK) */ total_size, free_size from __all_virtual_disk_stat where svr_ip = ? and svr_port = ?```</li><li>OceanBase V4.0 及以上版本：```select /* MONITOR_AGENT */ data_disk_capacity, (data_disk_capacity - data_disk_in_use) as data_disk_in_free, log_disk_capacity, log_disk_assigned, log_disk_in_use from V$OB_SERVERS where svr_ip = ? and svr_port = ?```</li></ul>  |
| 采集周期  | 5 秒    |
...
|--------------------|--------------------|----------|----------|---------|
| ob_disk_percent    | <li>ob_disk_percent > 97</li><li>ob_disk_percent > 85</li>  | 0 秒  | 20 秒 | 5 分钟 |

## 告警信息

|   告警触发方式   | 告警等级 | 范围 |
|------------------|----------|------|
| 基于监控指标的表达式 | <li>ob_disk_percent > 97：严重</li><li>ob_disk_percent > 85：警告</li>   | 主机 |
