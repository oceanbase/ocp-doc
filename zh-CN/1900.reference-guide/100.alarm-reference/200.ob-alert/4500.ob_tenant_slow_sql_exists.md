# ob_tenant_slow_sql_exists OceanBase 租户存在慢 SQL
...
|  参数   |    值    |
|-------|------|
| 监控指标  | max_elapsed_time_ms，该指标表示 OceanBase 租户执行 SQL 的最大耗时，当耗时大于阈值时触发告警。 |
| 监控表达式 | ```max(max_elapsed_time_ms)``` |
| 采集指标  | max_elapsed_time_ms  |
...
|        监控指标     | 默认阈值（单位：ms） | 持续时间 | 检测周期 | 消除周期 |
|---------------------|-------------|--------|------|------|
| max_elapsed_time_ms | 100         | 0 秒 | 20 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=obcluster-1:tenant_name=sys:sql_id=C52F76B2EED91902AFCD53ED906DA5A9 OceanBase 租户存在慢 SQL
