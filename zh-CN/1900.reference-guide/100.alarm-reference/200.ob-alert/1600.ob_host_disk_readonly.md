...
|  参数   |                           值                           |
|-------|-------------------------------------------------------|
| 监控指标  | disk_readonly_flag，该指标表示 OBServer 的安装目录、数据目录和日志目录所在盘是否存在只读，指标值为 `0` 时表示不存在只读；指标值为 `1` 时表示 OBServer 的安装目录、数据目录和日志目录的所在盘存在只读，并触发告警。       |
...
* **告警概述**
  ...
  * 样例：ob_cluster=obcluster-1:svr_ip=xxx.xxx.xxx.xxx 服务器磁盘只读
