# ob_host_disk_readonly OceanBase 服务器磁盘只读
...
**监控指标** disk_readonly_flag 的值表示 OBServer 的 安装目录、数据目录和日志目录所在盘是否存在只读。当监控指标值为 1 时触发告警，表示 OBServer 的安装目录、数据目录和日志目录的所在盘存在只读。
...
* **告警概述**
  ...
  * 样例：ob_cluster=obcluster-1:svr_ip=xxx.xxx.xxx.xxx OceanBase 服务器磁盘只读

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，主机：\${host}，告警：\${alarm_name}。磁盘挂载点 \${mount_point} 只读。
  * 样例：集群：obcluster-1，主机：xxx.xxx.xxx.xxx，告警：OceanBase 服务器磁盘只读。磁盘挂载点 /dev/loop0 只读。

* **告警恢复**

  * 模板：告警：${alarm_name}，OBServer 磁盘是否只读：\${recover_value}
  * 样例：告警：OceanBase 服务器磁盘只读，OBServer 磁盘是否只读：0
