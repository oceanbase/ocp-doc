...
# ob_host_disk_readonly 服务器磁盘只读

## 告警描述

OceanBase 服务器磁盘只读时将会触发该告警。
...
|  参数   |                           值                           |
|-------|-------------------------------------------------------|
| 监控指标  | disk_readonly_flag，该指标表示 OBServer 的安装目录、数据目录和日志目录所在盘是否存在只读，指标值为 `0` 时表示可写；指标值为 `1` 时表示 OBServer 的安装目录、数据目录和日志目录的所在盘存在只读，并触发告警。       |
| 监控表达式 | ```max(node_filesystem_readonly{@LABELS}) by (@GBLABELS)``` |
| 采集指标  | node_filesystem_readonly    |
| 指标来源  | 采集自 node_exporter。  |
| 采集周期  | 5 秒   |

OCP-Agent 每个 **采集周期** 通过定时任务监控磁盘是否可读，结果由 **采集指标** node_filesystem_readonly 返回，只读时为其传入 1，否则则传入 0。并为 OceanBase 安装目录、OceanBase 数据目录、OceanBase 日志目录所在盘标识 `is_ob_disk="1"`。
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=obcluster-1:host=xxx.xxx.xxx.xxx:mount_point=/home/admin 服务器磁盘只读

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，主机：\${host}，告警：\${alarm_name}。磁盘挂载点 \${mount_point} 只读。
  * 样例：集群：obcluster-1，主机：xxx.xxx.xxx.xxx，告警：服务器磁盘只读。磁盘挂载点 /dev/loop0 只读。
...
* 如果是 OceanBase 日志盘和数据盘只读，则 OBServer 无法正常运行。
