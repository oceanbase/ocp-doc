...
# ob_cluster_exists_index_fail_table OceanBase 集群存在索引失败的表
...
|  参数   |       值    |
|-------|---------|
| 监控指标  | index_fail_count，该指标表示 OceanBase 集群中索引失败的表的数量，当指标值大于阈值时触发告警。    |
| 监控表达式 | ```sum(ob_index_error_num{@LABELS}) by (@GBLABELS)```     |
| 采集指标  | ob_index_error_num   |
...
|-----------------------------|------|------|------|------|
| index_fail_count  | 0    | 0 秒  | 20 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=obcluster-1 OceanBase 集群存在索引失败的表

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，告警：\${alarm_name}。索引失败的表数量为 \${value}。
  * 样例：集群：obcluster-1，告警：OceanBase 集群存在索引失败的表。索引失败的表数量为 1.0。

* **告警恢复**

  * 模板：告警：${alarm_name}，OceanBase 集群索引不正常的表数量：\${value}
  * 样例：告警：OceanBase 集群存在索引失败的表，OceanBase 集群索引不正常的表数量：0
...
1. 根据下述方法找到失败的索引内容
   ...
   2. 通过 data_table_id（索引所属表的 ID）找到索引失败所属表名称。
      ...
      ```sql
      -- 注意 OceanBase V2.x 使用 __all_virtual_database
