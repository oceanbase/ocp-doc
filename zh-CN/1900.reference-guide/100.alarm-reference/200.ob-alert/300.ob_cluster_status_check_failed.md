# ob_cluster_status_check_failed OceanBase 集群状态检测失败
...
OCP 服务器通过定时任务检测 OceanBase 集群状态，该定时任务每 15s 执行一次，每次检测时会使用 OceanBase SDK 连接 OceanBase 集群，若连接失败则触发告警。

|  参数   |       值           |
|---------|---------------|
| 监控指标  | ob_cluster_status_check，该指标表示 OceanBase 集群状态是否检测失败，指标值为 `1` 时表示检测成功；指标值为 `0` 时表示检测失败，并触发告警。       |
| 监控表达式 | ```max(ob_cluster_status_check{@LABELS}) by (@GBLABELS)```    |
| 采集指标  | ob_cluster_status_check      |
| 指标来源  | 定时任务检查所有 OceanBase 集群的连接状态，通过尝试获取 OceanBase 版本来判断连接性。如果连接超时或者异常，则检测失败。  |
| 采集周期  | 5 秒      |
...
| -------- | -------- | -------- | -------- | -------- |
| ob_cluster_status_check     | 0      | 60 秒     | 15 秒    | 5 分钟   |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=obcluster-1 OceanBase 集群状态检测失败

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，告警：\${alarm_name}，集群状态：\${ob_cluster_status}，检查项：\${check_item}，失败原因：${failed_reason}
  * 样例：集群：obcluster-1，告警：OceanBase 集群状态检测失败，集群状态：Unavailable，检查项：cluster connect check，失败原因：some reason

* **告警恢复**

  * 模板：告警：\${alarm_name}，OceanBase 集群状态检测失败：\${value}
  * 样例：告警：OceanBase 集群状态检测失败，OceanBase 集群状态检测失败：1
...
* 如果 **<UI-TERM key="ocp-v2.Alarm.Event.Detail.AlarmDetails">告警详情</UI-TERM>** 中失败原因提示 Access Denied，则可能是 ocp_monitor@sys 账号被删除或该账号的密码被修改了。
...
* 如果 **<UI-TERM key="ocp-v2.Alarm.Event.Detail.AlarmDetails">告警详情</UI-TERM>** 中失败原因提示的是连接失败。
