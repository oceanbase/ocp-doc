...
OCP 服务器通过定时任务检测 OceanBase 集群状态，该定时任务每 30s 执行一次，每次检测时会使用 OceanBase SDK 连接 OceanBase 集群，若连接失败则触发告警。

|  参数   |       值           |
|-------|---------------|
| 监控指标  | ob_cluster_status_check       |
| 指标来源  | SQL：  |
...
* 告警概述样例：ob_cluster=obcluster-1 OceanBase 集群状态检测失败

* 告警详情样例：集群：obcluster-1，告警：OceanBase 集群状态检测失败，集群状态：Unavailable，检查项：cluster connect check，失败原因：some reason
