...
| 参数 | 值 |
| --- | --- |
| 监控指标 | arbitration_status_inactive_server_count，该指标表示与仲裁服务通信失败的 OBServer 节点个数，当其值大于阈值时触发告警。 |
| 监控表达式 | ```max(arbitration_status_inactive_server_count{@LABELS}) by (@GBLABELS)``` |
| 采集指标 | arbitration_status_inactive_server_count |
| 指标来源 | 采集自内部视图，采集 SQL：```select /*+ MONITOR_AGENT */ count(1) as inactive_server_count from GV$OB_ARBITRATION_SERVICE_STATUS where status = 'INACTIVE'```   |
| 采集周期 | 5 秒 |
...
| ---------- | ------- | ---------- | -------- | ---------- |
| arbitration_status_inactive_server_count | 0 | 0 秒  | 60 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：ob_cluster=TEST OceanBase 集群存在无法与仲裁服务通信的节点。
...
* **告警恢复**
  ...
  * 模板：告警：${alarm_name}，OceanBase 集群与其仲裁服务通信失败的节点个数：\${value_shown}
  * 样例：告警：OceanBase 集群存在无法与仲裁服务通信的节点，OceanBase 集群与其仲裁服务通信失败的节点个数：0

## 对系统的影响

OBServer 与仲裁服务无法通信，可能导致仲裁功能异常，无法提供给 2F/4F 租户高可用能力，当开启仲裁服务的租户半数副本不可用时，会导致租户服务异常。
