...
|-----------------|-----------|---------|----------|------|
| tenant_partition_leader_absent_count | 100     | 180 秒    | 20 秒    | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=TEST-1:tenant_name=t1 OceanBase 租户分区无主

* **告警详情**

  * 模板：集群：${ob_cluster_name}，租户：${tenant_name}，告警：${alarm_name}。
  * 样例：集群：TEST，租户：t1，告警：OceanBase 租户分区无主。

* **告警恢复**

  * 模板：告警：${alarm_name}，租户无主分区个数：\${recover_value}
  * 样例：告警：OceanBase 租户分区无主，租户无主分区个数：85
