...
由 Agent 判断 OBProxy 守护进程（`obproxyd.sh`）是否存在，OCP 每秒获取一次进程存在的状态。若进程不存在，则触发该告警。
...
|  参数   |  值       |
|-------|-----------|
| 监控指标  | obproxyd_process_exists，该指标表示 obproxy 的守护进程 `obproxyd.sh` 是否存在，指标值为 `1` 时表示存在；指标值为 `0` 表示不存在，并触发告警。   |
| 监控表达式 | ```process_exists{name="obproxyd.sh",obproxy_work_mode!="obSharding",@LABELS}```      |
| 采集指标  | obproxyd_process_exists      |
| 指标来源  | 该告警的指标来源相对特殊，是 OCP-Agent 执行如下 Linux 指令判断 OBProxy 守护进程是否存在：<br>```ps -ef\|grep -w obproxyd.sh\|grep -v grep\|wc -l``` |
| 采集周期  | 5 秒     |
...
|-------------------------|------------|------|------|------|
| obproxyd_process_exists | 0   | 30 秒  | 10 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：obproxy_cluster_id=3:obproxy_cluster=obproxy_02:svr_ip=xxx.xxx.xxx.xxx OBProxy 守护进程不存在

* **告警详情**

  * 模板：OBProxy 集群：\${obproxy_cluster}，主机：\${host}，告警：\${alarm_name}。
  * 样例：OBProxy 集群：obproxy_02，主机：xxx.xxx.xxx.xxx，告警：OBProxy 守护进程不存在。

* **告警恢复**

  * 模板：告警：${alarm_name}，OBProxy 守护进程存活状态：\${recover_value}
  * 样例：告警：OBProxy 守护进程不存在，OBProxy 守护进程存活状态：1
...
2. 使用 ssh 命令登录 OBProxy 机器看是否能正常登录。
   ...
   * 否，则 OBProxy 机器繁忙，建议重启 OBProxy。
     ...
     2. 在 OBProxy 列表中找到故障 OBProxy 主机，并单击其对应 **<UI-TERM key="ocp-v2.component.TraceTimelineDrawer.Operation">操作</UI-TERM>** 列的 **<UI-TERM key="ocp-v2.Detail.Component.RestartObproxyModal.Restart">重启</UI-TERM>** 。
