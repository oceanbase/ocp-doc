...
该告警需同时满足 OceanBase 版本和 OCP 版本要求：

1. OceanBase 版本：V3.2.3 BP10、V3.2.4 BP5、V4.1 BP3、V4.2.0 及之后版本。
2. OCP 版本：V4.2.2 及之后版本。

## 告警原理

| 参数      | 值      |
| --------- | ---------- |
| 监控指标 | ob_tenant_cpu_percent，该指标表示 OceanBase 租户的 CPU 使用率，当使用率大于阈值时触发告警。 |
...
| 指标来源 | 采集自内部视图，采集 SQL：<ul><li>OceanBase V4.0 之前版本：```select /*+ MONITOR_AGENT READ_CONSISTENCY(WEAK) */ con_id tenant_id, stat_id, value from v$sysstat where stat_id IN (140005, 140013) and (con_id > 1000 or con_id = 1)```</li><li>OceanBase V4.0 及之后版本：```select /* MONITOR_AGENT */ con_id tenant_id, stat_id, value from v$sysstat where stat_id IN (140005, 140013) and (con_id > 1000 or con_id = 1) and con_id in (select tenant_id from DBA_OB_TENANTS WHERE tenant_type<>'META')```</li></ul>当且仅当在 OceanBase V4.x 版本上开启了 cgroup 后（在 OceanBase V4.x 上，小于 V4.1 BP3 的版本只支持开启 cgroup 后获取 `cpu time` 值），会通过 cgroup 功能从 `cpuacct.usage` 中获取 CPU 时间；否则，不开启 cgroup 或者在 OceanBase V3.x 版本上，只会通过 `/proc/<pid>/task/<tid>/stat` 目录来读取 CPU 时间。 |
| 采集周期 | 5 秒 |
...
|--------------------------|------------|------|------|------|
| ob_tenant_cpu_percent    | 95         | 60 秒  | 10 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=TEST-1:tenant_name=t1:host=xxx.xxx.xxx.xxx OceanBase 租户 CPU 使用率超限
...
* **告警恢复**
  ...
  * 模板：告警：${alarm_name}，OceanBase 租户 CPU 使用率：\${value_shown}
  * 样例：告警：OceanBase 租户 CPU 使用率超限，OceanBase 租户 CPU 使用率：90 %
