...
| 参数     |   值                |
|---------|---------------------|
| 监控指标   | observer_boot_time_delta_seconds，该指标表示 observer 服务进程的启动时间戳变化，当其值大于阈值时触发告警。    |
...
|----------|---------------------|----------|----------|---------|
| observer_boot_time_delta_seconds   | 10   | 0 秒  | 20 秒 | 5 分钟 |
...
* **告警详情**
  ...
  * 样例：集群：AdminMETA，主机：xxx.xxx.xxx.xxx，告警：OBServer 进程停止。

* **告警恢复**

  * 模板：告警：${alarm_name}，OBServer 进程启动时间戳变化：\${recover_value}
  * 样例：告警：OBServer 进程停止，OBServer 进程启动时间戳变化：0 秒
