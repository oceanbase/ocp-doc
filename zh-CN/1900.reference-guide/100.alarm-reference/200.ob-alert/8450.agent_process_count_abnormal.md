...
| 参数 | 值 |
| --- | --- |
| 监控指标 | agent_process_count，该指标表示 ocp_agentd/ocp_mgragent/ocp_monagent 各进程的数量，当进程数量不为 `1` 时触发告警。 |
...
| ---------- | ------- | -------- | --------- | --------- |
| agent_process_count | != 1 | 60 秒 | 20 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:host=xxx.xxx.xxx.xxx:name=ocp_mgragent ocp-agent 进程数量异常。

* **告警详情**

  * 模板：集群：${ob_cluster_name}，主机：${host}，进程名：${name}，进程数量：${value}。
  * 样例：集群：TEST，主机：xxx.xxx.xxx.xxx，进程名：ocp_mgragent，进程数量：2。

* **告警恢复**

  * 模板：告警：${alarm_name}，进程数量：\${value}
  * 样例：告警：ocp-agent 进程数量异常，进程数量：1
