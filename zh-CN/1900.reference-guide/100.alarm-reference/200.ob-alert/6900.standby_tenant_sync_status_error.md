...
standby_tenant_restore_status_code == 1：租户的同步状态为同步异常
standby_tenant_restore_status_code == 2：租户的同步状态为日志缺失或日志冲突

## 规则信息

| 监控表达式 | 默认阈值 | 持续时间 | 检测周期 | 消除周期 |
| --- | --- | --- | --- | --- |
| standby_tenant_restore_status_code  | 该指标存在如下两个默认阈值：<li>1</li><li>2</li> | 0 秒  | 60 秒 | 5 分钟 |

该指标存在如下两个默认阈值：<li>1.5</li><li>2</li>
