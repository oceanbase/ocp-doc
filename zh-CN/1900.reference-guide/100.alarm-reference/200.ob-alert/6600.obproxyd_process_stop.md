...
| 参数     | 值                              |
|--------|---------------------------------|
| 监控指标 | obproxyd_boot_time_delta_seconds，该指标表示 OBProxy 守护进程的启动时间戳变化，当其值大于阈值时触发告警。    |
...
|------|-------------|------|------|------|
| obproxyd_boot_time_delta_seconds   | 10    | 0 秒  | 20 秒 | 5 分钟 |
...
* **告警详情**
  ...
  * 模板：OBProxy集群：${obproxy_cluster}，主机：${host}，告警：${alarm_name}。
  * 样例：OBProxy集群：ODPT2，主机：xxx.xxx.xxx.xxx，告警：OBProxy 守护进程停止。

* **告警恢复**

  * 模板：告警：${alarm_name}，OBProxy 守护进程启动时间戳变化：\${recover_value}
  * 样例：告警：OBProxy 守护进程停止，OBProxy 守护进程启动时间戳变化：0 秒
