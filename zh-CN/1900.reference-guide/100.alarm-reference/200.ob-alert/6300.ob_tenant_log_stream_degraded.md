...
# ob_tenant_log_stream_degraded OceanBase 租户存在已降级的日志流
...
| 参数 | 值 |
| ------- | ---------- |
| 监控指标 | ob_tenant_log_stream_degraded_count，该指标表示租户发生降级的日志流个数，当降级的日志流个数大于阈值时触发告警。 |
| 监控表达式 | ```sum(log_stream_degraded_count{@LABELS}) by (@GBLABELS)```     |
| 采集指标 | log_stream_degraded_count      |
| 指标来源 | 采集自内部视图，采集 SQL：```select /*+ MONITOR_AGENT */ t2.tenant_id, t2.tenant_name, sum(t1.degraded_list <> '') as degraded_count from V$OB_LOG_STAT t1 inner join DBA_OB_TENANTS t2 on t1.tenant_id = t2.tenant_id and t2.tenant_type != 'META' where svr_ip= ? and svr_port= ? group by t1.tenant_id``` |
| 采集周期 | 5 秒 |
...
| ---------- | ------- | -------- | -------- | -------- |
|  ob_tenant_log_stream_degraded_count | 0 | 0 秒 | 60 秒 | 5 分钟 |
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster_name=obcluste-4:tenant_name=mysql OceanBase 租户存在已降级的日志流
...
* **告警恢复**
  ...
  * 模板：告警：${alarm_name}，OceanBase 租户已降级的日志流个数：\${value_shown}
  * 样例：告警：OceanBase 租户存在已降级的日志流，OceanBase 租户已降级的日志流个数：0
