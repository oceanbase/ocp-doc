...
|  参数   |       值           |
|-------|---------------|
| 监控指标  | ob_cluster_sync，该指标表示获取 OceanBase 集群信息是否失败，当监控指标值为 1 时表示获取成功；当该值为 0 时表示获取失败，触发告警。       |
| 指标来源  | SQL：  |
...
* **告警概述**
  ...
  * 样例: ob_cluster=obcluster-1 OceanBase 集群同步失败
...
* **告警恢复**
  ...
  * 样例：OceanBase 集群同步失败，获取 OceanBase集群信息：1
