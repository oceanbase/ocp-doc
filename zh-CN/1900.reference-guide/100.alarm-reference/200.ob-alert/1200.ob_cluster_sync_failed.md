...
# ob_cluster_sync_failed 获取 OceanBase 集群信息失败
...
该告警的上报通过周期为 5 秒的定时任务实现，该定时任务的工作原理如下：
...
|  参数   |       值           |
|-------|---------------|
| 监控指标  | ob_cluster_sync，该指标表示获取 OceanBase 集群信息是否失败，指标值为 `1` 时表示获取成功；指标值为 `0` 时表示获取失败，并触发告警。       |
| 监控表达式 | ```max(ob_cluster_sync{ob_cluster_status=~"RUNNING\|UNAVAILABLE\|OPERATING",@LABELS}) by (@GBLABELS)```  |
| 采集指标  | ob_cluster_sync       |
| 指标来源  | OCP-Server 检查所有 OceanBase 集群的连接状态，如果连接超时或者异常，则检测失败。 |
| 采集周期  | 5 秒      |
...
| -------- | -------- | -------- | -------- | -------- |
| ob_cluster_sync      | 0       | 60 秒     | 15 秒    | 5 分钟   |
...
* **告警概述**
  ...
  * 样例: alarm_template_id=0:ob_cluster=obcluster-1 获取 OceanBase 集群信息失败

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，告警：\${alarm_name}，集群状态：\${ob_cluster_status}，失败原因：\${failed_reason}
  * 样例: 集群：obcluster-1，告警：OceanBase 集群同步失败，集群状态：Unavailable，失败原因：some reason

* **告警恢复**

  * 模板：告警：${alarm_name}，获取 OceanBase 集群信息：\${value_shown}
  * 样例：OceanBase 集群同步失败，获取 OceanBase 集群信息：1
