...
# ob_tenant_task_timeout OceanBase 租户任务超时
...
* **告警概述**
  ...
  * 样例：alarm_template_id=0:ob_cluster=C1-1000:tenant_name=T1:host=xxx.xxx.xxx.xxx:task_type=ALTER_RESOURCE_TENANT_UNIT_NUM OceanBase 租户任务超时

* **告警详情**

  * 模板：集群：\${ob_cluster_name}，租户：\${tenant_name}，主机：\${host}（zone：\${obzone}），告警：OceanBase 租户系统任务超时。任务类型：\${task_type}，执行时长 \${value_shown}，超过阈值 \${alarm_threshold} 秒。
  * 样例：集群：C1，租户：T1，主机：xxx.xxx.xxx.xxx（zone：Z1），告警：OceanBase 租户系统任务超时。任务类型：ALTER_RESOURCE_TENANT_UNIT_NUM，执行时长 4 小时，超过阈值 10800 秒。

* **告警恢复**

  * 模板：告警：${alarm_name}，OceanBase 租户系统任务运行时长：\${value_shown}
  * 样例：告警：OceanBase 租户系统任务超时，OceanBase 租户系统任务运行时长：2 小时
