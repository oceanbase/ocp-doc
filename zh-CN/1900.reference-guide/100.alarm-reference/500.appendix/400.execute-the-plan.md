...
| 告警名称 | 告警描述 | 预案说明 |
| -------- | -------- | -------- |
| os_observer_not_exist | observer 进程不存在 | 在 observer 进程异常退出时重新拉起，事件发生的 30 分钟之内仅拉起一次。 |
| arbitration_service_unavailable | 仲裁服务不可用 | 在仲裁服务的 observer 进程异常退出时重新拉起，事件发生的 30 分钟之内仅拉起一次。 |
...
2. 在告警详情页，了解告警基本信息后，单击预案 **<UI-TERM key="ocp-v2.Alarm.Event.Detail.ExecuteNow">立即执行</UI-TERM>**。
...
3. 在弹窗中单击 **<UI-TERM key="ocp-v2.Plan.Component.CreateDrawer.Confirm">确认</UI-TERM>**，执行预案。
...
4. 查看执行结果。
    ...
    2. 预案执行失败，进行预案重试或者预案回退。
        ...
        * 预案重试
          ...
          在查看执行结果界面，单击 **<UI-TERM key="ocp-v2.Alarm.Event.Detail.Retry">重试</UI-TERM>**。
...
5. 确认完成后可单击 **<UI-TERM key="ocp-v2.Backup.Backup.Log.Closed">关闭</UI-TERM>** 回到告警详情页。
