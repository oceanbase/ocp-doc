|slug|limit-the-inbound-traffic-of-the-oceanbase-cluster|
|---|---|
...
1. 找到该 OceanBase 集群执行频率高的 SQL。
   ...
   2. 在 **<UI-TERM key="ocp-v2.component.alarm.Search.Tenant">租户</UI-TERM>** 界面的 **<UI-TERM key="ocp-v2.Information.Detail.CollectDetails.TenantList">租户列表</UI-TERM>** 中找到该集群的租户，并单击租户名。

      系统默认进入 **<UI-TERM key="ocp-v2.Detail.Monitor.Monitor.Overview">总览</UI-TERM>** 页。

   3. 选择 **<UI-TERM key="ocp-v2.page.Iframe.SqlDiagnosis">SQL 诊断</UI-TERM>** ，随后选择 **TopSQL** 页签。
...
2. 为这些 SQL 增加并发执行限制的 Hint。
   ...
   2. 为这些 SQL 文本增加 HINT 索引 **`/*+max_concurrent(1)*/`** **，** 该索引将限制这个 SQL 文本的并发数为 1，从而限制该 SQL 的执行速度。可参考 [计划绑定](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000002016227) 或下述语句进行限流。
