...
* SQL 性能指标的数据结构
  ...
  | sqlType                     | String  | SQL 的类型，取值范围： <li>SELECT：查询语句</li><li>INSERT：插入语句</li><li>UPDATE：更新语句</li><li>DELETE：删除语句</li><li>REPLACE：替换语句</li><li>EXPLAIN：计划解析</li><li>UNKNOWN：其它语句</li>    |
  ...
  | avgLogicalReads          | Double   | 期间内 SQL 平均逻辑读次数。 |
  | cpuPercentage          | Double   | 期间该 SQL 在 TOPSQL 中的 CPU 时间占比。 |
  | dynamicSql          | Boolean   | 是否是动态 SQL。 |
  | lastFailCode          | Integer   | 期间最后一次的错误码。 |
  | lastFailTimestamp          | Integer   | 期间最后一次的错误时间。 |
  | obDbId          | Integer   | OceanBase 数据库 ID。 |
  | obTenantId          | Integer   | OceanBase 租户ID。 |
  | serverIp          | String   | OBServer IP。 |
  | sumCpuTimeMs          | Double   | 期间的总 CPU 消耗。 |
  | sumElapsedTime          | Double   | 期间内的总响应时间（毫秒）。 |
  | sumLogicalReads          | Integer   | SQL 的总逻辑读次数。 |
  | sumWaitTime          | Double   | 期间内的总等待时间（毫秒）。 |
