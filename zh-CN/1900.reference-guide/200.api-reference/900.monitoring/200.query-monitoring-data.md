...
| groupBy    | StringArray | 是  | app,obregion,ob_cluster_id | 监控数据的汇总标签。监控数据的汇总标签。 <br>其中：<li>app：监控数据采集的来源。可以取值为 `host`、`ob` 或 `obproxy`，分别代表主机监控、OceanBase 监控和 OBProxy 监控。</li> <li>obregion：即 ob_cluster_name，监控的 OceanBase 集群名称。</li> <li>ob_cluster_id：监控的 OceanBase 集群 ID。</li>      |
