...
| architecture             | String | 否  | x86_64  | CPU 架构。 |
| packageOperatingSystem   | String | 否  | xx      | 集群软件包操作系统，未填写则默认使用集群配置。 |
| startupParameters Object | Object | 否  | -       | 启动参数。 |
| ├─name                   | String | 否  | -       | 参数名。 |
| ├─value                  | String | 否  | -       | 参数值。 |
| addReplicaParams         | Array  | 否  | -       | 租户副本配置相关参数。 |
| ├─zoneName               | String | 是  | zone1   | Zone 名称。 |
| ├─replicaType            | Enum   | 是  | FULL    | 副本类型。 |
| ├─resourcePool           | Object | 是  | -       | 资源池信息。 |
|   ├─unitSpecName         | String | 是  | S1      | Unit 规格名。 |
|   ├─unitCount            | Long   | 是  | 1L      | Unit 数量。 |
| ├─tenantId               | Long   | 是  | 1L      | 租户 ID。 |
| ├─tenantName             | String | 否  | tenant1 | 租户名称。 |
