...
| dbPrivileges     | Array       | 否  |                    | 要授予的数据库权限列表，仅 MySQL 租户中存在，Oracle 租户无此字段。  |
| ├─ dbName        | String      | 是  | db1                | 数据库名。   |
| ├─ privileges    | StringArray | 是  | \[ALTER, INDEX\]   | 数据库的权限列表。   |
| roles            | StringArray | 否  | \[R1, R2\]         | 要授予的角色列表，仅 Oracle 租户中存在，MySQL 租户无此字段。    |

### 返回结果

|     参数      |    类型    |           说明           |
|-------------|----------|------------------------|
| data        | Object   | 数据库用户的列表，详情参见 **数据库用户数据结构**。          |
...
| dbPrivileges            | Array       | 数据库权限列表，仅 MySQL 租户中存在，Oracle 租户无此字段。    |
| ├─ dbName               | String      | 数据库名。      |
| ├─ privileges           | StringArray | 数据库的权限列表。   |
| objectPrivileges        | Array       | 对象权限列表，仅 Oracle 租户中存在，MySQL 租户无此字段。   |
...
| grantedRoles            | StringArray | 用户被授予的角色，仅 Oracle 租户中存在，MySQL 租户无此字段。   |
...
| accessibleDatabases     | StringArray | 可访问的数据库，仅 MySQL 租户中存在，Oracle 租户无此字段。    |
