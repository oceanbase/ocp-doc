...
OCP 产生告警后，告警消息通过告警通道发送出去，通道可配置 Shell 脚本和 Python 脚本。该示例描述如何将告警通过通道发送出去。`send_alarm` 函数通过 os.environ 从环境变量获取告警相关的变量值。

以下为 Python 脚本示例，更多示例可参考 [查看告警通道配置示例](../../../860.alert-management/500.manage-alert-channels/600.examples-of-alert-channel-configuration.md)。
...
* OCP 通道设置中有测试功能，可直接通过 **<UI-TERM key="ocp-v2.Alarm.Channel.New.SendTestMessages">发送测试消息</UI-TERM>** 按钮进行测试。参考 [新建告警通道](../../../860.alert-management/500.manage-alert-channels/100.create-an-alert-channel.md)。
...
将 OCP 的告警接入用户告警平台时，需使用查询告警事件的 API，详情参见 [获取当前告警](200.fetch-actived-alarms.md) 或 [获取历史告警](300.fetch-history-alarms.md)。
