...
| obLinks             | Array   | 是  | --                                     | 可连接 OceanBase 集群信息，详情参见 **ObLink 数据结构**。RS_LIST 模式下有且只能有一个可连接 OceanBase 集群。CONFIG_URL 模式下无限制。        |
| startupParameters   | Array   | 是  | --                                     | OBProxy 启动参数，详情参见 **ObproxyParameter 数据结构**。可以为空数组。   |
| parameters          | Array   | 是  | --                                     | OBProxy 非启动参数，详情参见 **ObproxyParameter 数据结构**。可以为空数组。   |
...
| obClusterId | Integer | 是  | 1       | 集群在 OceanBase 端的 ID。 |
