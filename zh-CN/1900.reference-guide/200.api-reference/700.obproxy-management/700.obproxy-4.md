...
| 参数 |  类型  | 必选 | 示例值 |      描述       |
|---------|---------|--------|---------|---------------|
| id      | Long    | 是     | 33001   | OBProxy 集群 ID。 |

**body：**

|  参数   |  类型   | 必选 | 示例值  |         描述          |
|-------|-------|---------|----------------------|
| hosts        | Array   | 是  | -      | 待接管的 OBProxy 所在主机信息，详情参见 **ObproxyTakeOverParam 数据结构**。 |
| rpcPort      | Integer | 否  | 2885   | 待接管的 OBProxy 的 RPC 端口，当 OBProxy 为 V4.3.0 及以上版本时必填。 |
| exporterPort | Integer | 否  | 2884   | 待接管的 OBProxy 的 Exporter 端口。 |
...
| sqlPort | Integer | 是  | 2883 | 待接管的 OBProxy 的 SQL 端口。 |
