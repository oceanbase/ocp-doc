...
|    参数    |   类型    | 必选 | 示例值 |  描述   |
|----------|---------|----|-----|-------|
| id       | Integer | 是  | 1   | OCP 侧的集群 ID。 |
| tenantId | Integer | 是  | 1000001   | OCP 侧的租户 ID。 |
...
|-------------|----------|-----------------------|
| data        | Object   | 请求的业务数据。          |
| ├─ contents | Array    | 数据库的列表，详情参见 **数据库数据结构**。    |
...
| ├─ connectionStringType | String   | 连接串类型，取值范围：<li>OBPROXY：OBProxy</li><li>DIRECT：直连</li>    |
...
| ├─ forBinlogService     | Boolean  | 是否服务于 Binlog 服务。        |
