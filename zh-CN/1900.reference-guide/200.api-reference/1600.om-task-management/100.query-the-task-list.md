...
| clusterId  | Long        | 否  | 100            | OCP 侧集群 ID。    |
| tenantName | String      | 否  | SYS            | 租户名称，需配合 `clusterId` 一起使用。    |
| tenantId   | Long        | 否  | 100            | OCP 侧租户 ID。     |
...
| subtasks   | ObjectArray | BasicSubtaskInstance 数组，子任务汇总信息，详情参见 **BasicSubtaskInstance 数据结构**。    |
