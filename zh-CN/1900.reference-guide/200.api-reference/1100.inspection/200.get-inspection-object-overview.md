...
|  参数  |  类型  |  必选  |  示例值  |  描述  |
|--------|--------|--------|----------|--------|
|  objectIds  |   Array  |  否   | 1   |  巡检对象在 OCP 中的 ID，如集群 ID、租户 ID，多个 ID 时使用 `,` 分隔。  |
...
|  scheduleStates  |  Array  |  否  |  ACTIVE  |  调度状态，包括：<li>已启用：ACTIVE。</li><li>已关闭：INACTIVE。</li><li>已过期：EXPIRED。</li>  |
...
|--------|--------|----------------------------------|
|  data        |  Object   | 请求的数据。 |
|  ├─ contents |  Array    | 详情参见 **InspectionObjectOverview 数据结构**。    |
