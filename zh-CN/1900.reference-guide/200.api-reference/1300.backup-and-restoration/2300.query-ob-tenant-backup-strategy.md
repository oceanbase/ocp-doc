...
|  参数               |  类型          |  说明  |
|---------------------|----------------|----|
|  backupStorageType  |  String        |  存储类型，目前支持 `BACKUP_STORAGE_FILE`、`BACKUP_STORAGE_OSS`、`BACKUP_STORAGE_S3` 类型。  |
...
|  customBackupRangeList       |  Array  |  自定义备份范围，支持的 OBServer 版本: [4.2.5.2, 4.3.0) 和 [4.2.1.11, 4.2.2)，详情参见 **`customBackupRange` 的数据结构** 。  |
...
**`customBackupRange` 的数据结构**

|  参数             |  类型    |  说明         |
|-------------------|----------|---------------|
|  backupType      |  Enum  |  必选参数。备份类型，包括：DATA 和 LOG。  |
|  nodeType  |  Enum  |  必选参数。备份节点类型，包括：ZONE、REGION、IDC。  |
|  nodeList           |  Array  |  必选参数。备份节点集合。同一下标下的节点优先级相同，同一下标下的多个节点用逗号隔开，例如: ["zone1,zone2", "zone3"]。  |
