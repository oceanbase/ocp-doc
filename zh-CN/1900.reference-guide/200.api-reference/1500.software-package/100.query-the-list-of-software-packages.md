...
| type            | StringArray | 否  | OB_SERVER_INSTALL_PACKAGE,OB_AGENT_INSTALL_PACKAGE | 软件包类型字段，支持多种类型过滤，有如下候选值： <li>`OB_SERVER_INSTALL_PACKAGE`：OceanBase 安装包。</li><li>`OCP_AGENT_INSTALL_PACKAGE`：OCP Agent 安装包。</li><li>`OBPROXY_INSTALL_PACKAGE`：OBProxy 安装包。</li><li>`OB_BACKUP_AGENT_INSTALL_PACKAGE`：备份恢复安装包。</li><li>`BINLOG_SERVICE_INSTALL_PACKAGE`：Binlog Service 安装包。</li><li>`OTHERS_RPM`：其它安装包。</li>  |
...
|------------------|----------|-----------------------------------|
| data             | Object   | 请求的数据。                             |
| ├─ contents      | Array    | SoftwarePackageMeta 的数组，详情参见 **SoftwarePackageMeta 的数据结构**。 |
| ├─ page          | Object   | 分页信息。                              |
...
| type            | String  | 软件包类型：<li>`OB_SERVER_INSTALL_PACKAGE`：OceanBase 安装包。</li><li>`OCP_AGENT_INSTALL_PACKAGE`：OCP Agent 安装包。</li><li>`OBPROXY_INSTALL_PACKAGE`：OBProxy 安装包。</li><li>`OB_BACKUP_AGENT_INSTALL_PACKAGE`：备份恢复安装包。</li><li>`BINLOG_SERVICE_INSTALL_PACKAGE`：Binlog Service 安装包。</li><li>`OTHERS_RPM`：其它安装包。</li>    |
