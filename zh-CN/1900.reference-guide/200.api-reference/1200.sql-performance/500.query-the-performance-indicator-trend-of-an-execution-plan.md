...
* 计划性能采样的数据结构
  ...
  | largeQueryPercentage        | Double    | 期间内的大查询百分比                     |
  | timeoutCount        | Integer    | 期间内的超时次数                     |
  | delayedLargeQueryPercentage | Double    | 期间内被置入队列的大查询的百分比               |
  | timeoutPercentage           | Double    | 期间内执行超时的百分比                    |
