...
| 参数 | 类型    | 必选 | 示例值 | 描述           |
|------|---------|------|--------|-----------------|
| id   | Integer | 是   | 3000010      | 主租户所属集群在 OCP 侧的集群 ID。 |
...
| 参数 | 类型 | 必选 | 示例值 | 描述 |
|---------|---------|---------|---------|---------|
| tenantId        | Long | 是 | 2000025 | 主租户在 OCP 侧的租户 ID。 |
| standbyTenantId | Long | 是 | 2000022 | 备租户在 OCP 侧的租户 ID。 |
| standbyOcpClusterId | Long   | 是   | 100001   | 备租户所在的 OCP 集群 ID。 |
...
|----------|----------|-----------|
| valid       | Boolean | 预检查结果。 |
| tenantId    | Long    | 主租户在 OCP 侧的租户 ID。 |
| name        | String  | 主租户名。 |
| clusterId   | Long    | 主租户所属集群在 OCP 侧的集群 ID。 |
...
| tenantId     | Long    | 备租户在 OCP 侧的租户 ID。 |
| name         | String  | 备租户名。 |
| clusterId    | Long    | 备租户所属集群在 OCP 侧的集群 ID。 |
