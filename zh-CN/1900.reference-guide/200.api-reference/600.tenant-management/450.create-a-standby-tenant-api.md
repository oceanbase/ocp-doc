...
| 参数 | 类型    | 必选 | 示例值 | 描述           |
|------|---------|------|--------|-----------------|
| id   | Integer | 是   | 3000010      | 目标租户所属集群在 OCP 侧的集群 ID。 |
...
| primaryTenantId   | Integer     | 是 | 1 | 主租户主键 ID，即主租户在 OCP 侧的租户 ID。 |
...
| zones             | ZoneParam   | 是 | - | 详情参见 **ZoneParam 数据结构**。 |
| parameters        | TenantParameterParam | 否 | - | 详情参见 **TenantParameterParam 数据结构**。 |
...
| 参数              | 类型    | 必选   | 示例值   | 描述  |
|-------------------|---------|-------|----------|-------|
| backupStorageType | Enum    | 是    | BACKUP_STORAGE_FILE | <li>BACKUP_STORAGE_FILE：NFS 存储。<li></li>BACKUP_STORAGE_OSS: OSS 存储。<li></li>BACKUP_STORAGE_S3: S3 存储。</li> |
| dataBackupUrl     | String  | 是    | `/obbackup/clusterA/tenantA/data` | 数据备份目录。 |
| logBackupUrl      | String  | 是    | `/obbackup/clusterA/tenantA/clog` | 日志备份目录。 |
| ossSecretModel    | ObjectStorageSecretParam       | 否    | - | 对象存储密钥信息，仅存储模式为 OSS/S3 时需要传该参数，详情参见 **ObjectStorageSecretParam 数据结构**。 |
