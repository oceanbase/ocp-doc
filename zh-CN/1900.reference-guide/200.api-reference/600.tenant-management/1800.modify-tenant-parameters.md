...
|    参数    |  类型  | 必选 | 示例值 |      描述       |
|----------|------|------|------|---------------|
| id       | Long | 是  | 1   | 目标租户所属集群在 OCP 端的集群 ID。 |
| tenantId | Long | 是  | 12  | 目标租户在 OCP 端的租户 ID。     |

**body：**

|          参数          |  类型   | 必选 | 示例值 |            描述            |
|----------------------|-------|------|------|--------------------------|
| TenantParameterParam | Array | 是  | -  | TenantParameterParam 列表，详情参见 **TenantParameterParam 数据结构**。 |
