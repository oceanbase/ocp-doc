...
| mode      | String      | 否  | MYSQL    | 查询指定模式的租户，取值范围：<li>ORACLE：Oracle 模式</li><li>MYSQL：MySQL 模式</li>  |
| locked    | Boolean     | 否  | true     | 是否锁定。    |
| readonly  | Boolean     | 否  | Boolean  | 是否只读。   |
| status    | StringArray | 否  | CREATING | 租户的状态，取值范围：<li>NORMAL：运行中</li><li>CREATING：创建中</li><li>MODIFYING：修改中</li><li>DELETING：删除中</li>    |
...
|------------------|----------|-----------------------|
| data             | Object   | 请求的业务数据。               |
| ├─ contents      | Array    | 租户信息的数据，详细参见 **租户信息数据结构**。     |
| ├─ page          | Object   | 分页信息。                 |
