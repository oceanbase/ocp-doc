...
|    参数     |  类型  | 必选 | 示例值 |      描述       |
|-----------|------|----|-----|---------------|
| id        | Long | 是  | 1000014   | 目标租户所属集群在 OCP 侧的集群 ID。 |
| tenantId  | Long | 是  | 1000007   | 目标租户在 OCP 侧的租户 ID。      |
...
| status      | String   | 租户的状态。 取值范围：<li>NORMAL：正常。</li><li>UNAVAILABLE：不可用。</li><li>CREATING：创建中。</li><li>OPERATING：运维中。</li><li>MODIFYING：修改中。</li><li>CLONING：克隆中。</li><li>RESTORING：租户恢复中。</li><li>DISCONNECTING：解耦。</li><li>FAILOVER：容灾切换。</li><li>SWITCHOVER：日常切换。</li><li>MOVED_OUT：已迁出。</li><li>TDE_ENABLING：正在启用透明数据加密（TDE）。</li>    |
...
| zones       | Array    | 详情参见 **TenantZone 数据结构**。       |
