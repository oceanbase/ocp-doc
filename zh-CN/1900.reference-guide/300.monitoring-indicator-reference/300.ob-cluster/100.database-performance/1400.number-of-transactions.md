...
本节为您介绍 OceanBase 集群的性能指标 **事务数量** ，**事务数量** 指标用于统计 OceanBase 集群每秒处理事务的次数。该指标细分为 2 个子指标对集群的事务数量进行统计，包括：trans_single_partition_count（每秒普通事务数）、trans_multi_partition_count（每秒分布式事务数）。您可指定统计范围，查询 OceanBase 集群在单个 Zone 或 OBServer 节点的 **事务数量** 。
...
|-----------|----------|--------|
| trans_single_partition_count | transaction_single_partition_count | times/s  |
...
|------------|---------|--------|
| trans_multi_partition_count | transaction_multi_partition_count | times/s      |
