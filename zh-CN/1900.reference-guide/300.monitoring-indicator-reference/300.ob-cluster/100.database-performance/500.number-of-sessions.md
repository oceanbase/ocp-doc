|slug|number-of-sessions|
|---|---|

# 会话数

本节为您介绍 OceanBase 集群的性能指标 **<UI-TERM key="ocp-v2.src.constant.monitor.Sessions">会话数</UI-TERM>** 。该指标统计集群的当前会话数（all_session）和当前活跃会话数（active_session）。您可指定统计范围，查询 OceanBase 集群在单个 Zone 或 OBServer 节点的 **<UI-TERM key="ocp-v2.src.constant.monitor.Sessions">会话数</UI-TERM>** 。
