|slug|tenant500-memory|
|---|---|

# 租户 500 内存占用

本节为您介绍 OceanBase 集群的性能指标 **租户 500 内存占用** 。您可指定统计范围，查询 OceanBase 集群在单个 Zone 或 OBServer 节点的 **租户 500 内存占用** 。

## tenant500_memory

### 指标介绍

统计范围内，集群租户 500 内存占用大小（不包括 OB_KVSTORE_CACHE_MB）。
...
统计范围内，集群租户 500 内存使用（不包括 OB_KVSTORE_CACHE_MB） / OBServer 内存资源的大小。
