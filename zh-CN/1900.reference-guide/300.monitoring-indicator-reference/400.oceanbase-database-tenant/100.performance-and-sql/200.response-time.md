...
histogram_quantile(0.99, sum(rate(ob_query_rt_total_cumulative_count{@LABELS}[@INTERVAL])) by (@GBLABELS,le))
...
histogram_quantile(0.95, sum(rate(ob_query_rt_total_cumulative_count{@LABELS}[@INTERVAL])) by (@GBLABELS,le))
