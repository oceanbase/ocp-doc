...
本节为您介绍租户的性能指标 QPS（Query Per Second）。 包括分别针对 SELECT 语句，INSERT 语句、UPDATE 语句、REPLACE 语句、DELETE 语句、COMMIT 语句、ROLLBACK 语句的指标及针对其它类型的语句的细分指标。您可指定统计范围，查询租户在单个 Zone 或 OBServer 节点的 **QPS** 数据。
