...
* 当监控查询时间范围较大时，系统会对时序数据采集降低采集频率，根据特定算法的聚合展示相关监控指标的平均值（Avg）和最大值（Max），通过减少数据量来延长存储时间，提升查询速度并尽可能地保留监控数据的趋势和特征。系统默认展示监控指标的平均值（Avg），您可以通过修改 `ocp.monitor.metric.collect.rollup.type` 参数，来决定是否展示为最大值（Max）或初始值（FIRST）。
