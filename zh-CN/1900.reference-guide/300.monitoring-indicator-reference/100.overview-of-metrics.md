...
* 当监控查询时间范围较大时，系统会对采集的源数据进行聚合，通过秒级数据降采样到分钟级、分钟级数据降采样到小时级的降采样逻辑，来减少数据量并延长存储时间，从而在提升查询速度的同时尽可能地保留监控数据的趋势和特征。系统默认展示监控指标的最大值（Max），即用秒级数据在这一分钟内的最大值作为分钟级数据当前分钟点的数据，您可以通过修改 `ocp.monitor.metric.collect.rollup.type` 参数，来决定是否展示为平均值（Avg）或初始值（FIRST）。
