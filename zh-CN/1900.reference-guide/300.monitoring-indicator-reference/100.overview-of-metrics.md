...
| 计算表达式  | 表示该指标的数据来源。  |
|SQL 采集|监控指标采集的 SQL 来源。不同 OceanBase 版本的 SQL 采集有所不同，您可根据实际情况进行查看。<main id="notice" type='explain'><h4>说明</h4><p><li>部分监控指标不涉及 SQL 采集，此类指标的说明中将无 SQL 采集相关内容。</li><li>若 SQL 采集语句中包含主机 IP 及端口信息，如 <code>where svr_ip = ? and svr_port = ?</code>，您在执行该语句时需将 <code>?</code> 替换为实际的主机 IP 及 RPC 端口号，端口号默认为 2882。</li></p></main>|
