...
* 将待接管 OBProxy 的可连接 OB 集群，添加到目标 OBProxy 集群的可连接 OB 集群列表中，方法参见 [添加可连接的 OB 集群](/zh-CN/3.ob-cloud-platform/8.obproxy-management/10.add-a-connectable-ob-cluster.md)。然后再对待接管集群进行接管。
