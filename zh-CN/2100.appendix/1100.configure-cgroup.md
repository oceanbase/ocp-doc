...
OceanBase 数据库的当前版本中，由于租户工作线程和大部分后台线程均根据租户来区分，网络线程为共享线程，用户可以通过配置 cgroup 来控制租户 CPU 或 IOPS 等资源的占用。在开始配置 cgroup 前，建议您先了解 cgroup 的概念，有关 cgroup 的介绍信息请参见 [资源隔离概述](../700.tenant-functions/1250.manage-resource-isolation/100.resource-isolation-overview.md)。
...
2. 执行以下命令，挂载 `/sys/fs/cgroup` 目录。
   ...
   <h4>说明</h4>
   <p>如果已经存在 <code>/sys/fs/cgroup</code> 目录，可忽略该步骤。</p>
   </main>
...
如果未开启 cgroup 功能，您可以参考以下操作开启 cgroup 功能。
...
建议使用 cgroup 中的目录文件调用 `DBMS_RESOURCE_MANAGER` 系统包中的子程序 `CREATE_CONSUMER_GROUP` 创建的资源组，来进行用户级或 SQL 级的资源隔离。配置资源隔离的详细操作，参见 [创建资源组](../700.tenant-functions/1250.manage-resource-isolation/200.create-a-resource-group.md) 和 [创建资源隔离计划](../700.tenant-functions/1250.manage-resource-isolation/300.create-a-resource-isolation-plan.md)。

## 相关文档

[清除 cgroup 配置](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000004479249)
