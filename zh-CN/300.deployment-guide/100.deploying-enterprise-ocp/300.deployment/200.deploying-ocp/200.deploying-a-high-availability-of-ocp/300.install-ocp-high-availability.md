...
4. 在 **安装 OCP** 页面，配置各项参数。
   ...
   1. 配置 OCP 的基础信息。
        ...
        |   管理员密码  |    对 OCP admin 账号的密码进行重置，可以自定义密码，也可以随机生成。待 OCP 部署完成后，您可直接使用该密码登录 OCP，无需再进行 **设置密码激活账号** 操作。 <blockquote>**说明**</br>仅 OAT V4.1.1 及以上版本支持配置管理员密码。</blockquote>      |
        |   多可用区模式 |   选择是否启用多可用区安装。<blockquote>**说明**</br>当 OCP 为 V4.3.0 及以上版本时，不再支持部署为多可用区模式，请勿开启此项配置。</blockquote>     |
