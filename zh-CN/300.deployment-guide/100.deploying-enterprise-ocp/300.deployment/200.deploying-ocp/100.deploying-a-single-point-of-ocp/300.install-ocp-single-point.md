...
4. 在 **安装 OCP** 页面，配置各项参数。
   ...
   1. 配置 OCP 的基础信息。
        ...
        |   管理员密码  |    对 OCP admin 账号的密码进行重置，可以自定义密码，也可以随机生成。待 OCP 部署完成后，您可直接使用该密码登录 OCP，无需再进行 **设置密码激活账号** 操作。 <blockquote>**<UI-TERM key="ocp-v2.Settings.Credential.OBProxy.Description">说明</UI-TERM>**</br>仅 OAT V4.1.1 版本支持在安装 OCP 时配置管理员密码。</blockquote>      |
...
5. OCP 配置完成后，单击 **<UI-TERM key="ocp-v2.component.OperationConfig.ModifyParameterDrawer.Submit">提交</UI-TERM>**。若您的配置有误，可单击 **<UI-TERM key="ocp-v2.page.QueryLog.Reset">重置</UI-TERM>** 一键恢复默认配置。

6. 提交安装 OCP 成功后，进入 **<UI-TERM key="ocp-v2.src.hook.useMenuOptions.TaskCenter">任务中心</UI-TERM>** 页面，单击任务名称，可查看详细任务信息。
