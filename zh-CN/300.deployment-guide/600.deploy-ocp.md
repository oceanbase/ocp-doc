...
已参见 [安装准备](../300.deployment-guide/400.installation-preparation.md) 获取 `ocp-server-all-in-one-4.0.3.0-YYYYMMDDhhmmss.el7.x86_64.tar.gz` 软件包，将软件包上传至 OCP 宿主机的任一目录，并执行如下命令进行解压。

```bash
tar zxf ocp-server-all-in-one-4.0.3.0-YYYYMMDDhhmmss.el7.x86_64.tar.gz
```
...
4. 单击 **下一步**，校验当前输入的配置。
    ...
    * 若配置检查通过，在弹窗中对 OCP 集群进行资源分配，并单击 **确定**。
        ...
        | OCP-Server   | <ul><li>CPU：默认 1C，最小 1C，支持调整，调整上限为剩余的核心数。</li><li>下限：1 GB。</li><li>内存：默认 1GB，最小 1GB，支持调整，调整上限为剩余的内存数，如果上限超过 5GB，则默认 5GB。</li></ul>  |
        | 元信息租户   | <ul><li>CPU：默认 2C，最小 1C，支持调整，调整上限为剩余的核心数。</li><li>内存：默认4GB，最小 2GB，支持调整，调整上限为剩余的内存数。</li></ul>  |
        | 监控数据租户   | <ul><li>CPU：默认 4C，最小 2C，支持调整，调整上限为剩余的核心数。</li><li>内存：默认8GB，最小 4GB，支持调整，调整上限为剩余的内存数。</li></ul>  |
