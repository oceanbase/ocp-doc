...
2. 通过 Docker 部署 OpenSearch。
    ...
    | HOST_IP | 选填 | 主机 IP，请根据实际 IP 地址进行填写。如果不指定，将通过 `hostname -i` 获取，需要保证 `/etc/hosts` 中正确配置。 |
...
3. OCP 连接 OpenSearch。
    ...
    在 OCP 启动后，需要在 **<UI-TERM key="ocp-v2.src.util.menu.SystemParameters">系统参数</UI-TERM>** 中手动添加 OpenSearch 的地址和账密。详情可参考 [配置链路查询相关参数](../1300.log-service/300.configuration-trace-paremeters.md) 。
