...
**Q2: OCP 是否支持 OceanBase 主集群为 X86 ，备集群为 ARM 的部署规划？**
...
1. 查看 OceanBase 集群内 **<UI-TERM key="ocp-v2.src.util.menu.TenantManagement">租户管理</UI-TERM>** 界面中的 **容量_分区数量** 图中是否显示相关数据，结果未显示。

2. 查看对应主机的 **<UI-TERM key="ocp-v2.Host.Detail.OCPAgent.Processes">进程列表</UI-TERM>** 中进程是否正常，发现 obstat 和 logtailer 进程异常停止。
...
**Q12：升级 OCP 后丢失了某个 OceanBase 集群的 proxyro 密码配置，导致该集群不可用，且重启集群失败，要如何处理？**
...
**Q8: 新建集群在 Do io bench 子步骤失败，如何排查？**

A: 观察 `io bench` 失败返回的错误码，若错误码为 87，通常是因为主机资源不足导致的，需保证磁盘有 100GiB 的可用空间。
...
A：在主集群和备集群的概览页面，右上角都有 **<UI-TERM key="ocp-v2.Topo.TopoGraph.UpgradedVersion">升级版本</UI-TERM>** 的按钮。无论是主集群还是备集群的升级入口，都可以对整个主备集群发起升级操作。
...
**Q3: OCP 中主备库的异步传输日志延迟是如何统计的？**

A: OCP 主备库异步传输日志延迟的数据来源如下：
...
即 `(time_to_usec(now(6)) - current_scn)/1000000`，单位为秒。
