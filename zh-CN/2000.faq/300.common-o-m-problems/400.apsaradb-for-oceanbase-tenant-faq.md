...
**Q2: OceanBase 集群有 3 个 Zone，使用 OCP 成功删除某一租户下的一个副本，然后在该租户下再新增副本报错 `操作 OB 失败，错误信息：tenant already exist`，如何解决？**

A: 租户删除副本是调用 OceanBase 的 `drop resource pool` 命令删除资源池。但删除资源池后，还需要异步删除资源池中的 Unit。当再次新增副本时，因为租户中的 Unit 还未删除完成，导致 `tenant already exist` 的报错。您可等待删除中的 Unit 删除完成后，重试当前操作。

可参考如下两种方式观察 Unit 是否删除完成：

* 当 OceanBase 为 V4.0.0.0 及以上版本时，查询 `GV$OB_UNITS` 视图，若视图中不存在 Unit 的数据表示 Unit 已经删除完成。
* 当 OceanBase 为 V4.0.0.0 以下版本时，您可通过查询 `__all_virtual_partition_info`，观察机器上是否还有租户的副本来判断 Unit 是否删除完成。
...
**Q7：黑屏修改租户名后，OCP 显示租户不可用，并收到了相关告警，该问题出现的原因和解决办法是什么？**

A：黑屏修改租户名后，OCP 无法识别，因此会显示租户不可用，并发送相关告警信息。您可黑屏用同样方式将租户修改回原来的名字，再使用 OCP 提供的租户 [重命名](../../700.tenant-functions/600.manage-a-tenant/250.rename-a-tenant.md) 功能。
...
A：执行 `vi /home/admin/obztools_agent/src/task/obauto_plus/ob_operate_plus.py` 修改 ob_operate_plus.py 文件。
...
A: 在租户的 **<UI-TERM key="ocp-v2.Detail.Monitor.Monitor.Overview">总览</UI-TERM>** 页面，可以调整租户副本的 Unit 规格（纵向扩容 scale up），也可以调整租户副本在当前 Zone 下的 Unit 个数（水平扩容 scale out）。

**Q2: 租户缩容时，是由 OCP 确定要删除哪一个 Unit 吗？能否删除指定的 Unit？**

A: 不是，由管理员选择。进入到租户的 **<UI-TERM key="ocp-v2.src.util.menu.TopologicalGraph">拓扑图</UI-TERM>** 页面，选中指定的 Unit 并删除。
