|slug|monitor-dashboard-overview|
|---|---|
...
在监控大盘页面，您可以创建多个监控大盘页签。在每个监控大盘页签中，您可以添加多个图表来显示不同的指标和数据，诸如用户多实例、多指标监控视图的关联查看和单个或者多个集群的不同监控指标等。同时，监控大盘提供如 **<UI-TERM key="ocp-v2.widgets.Chart.CompareModal.RingRatio">环比</UI-TERM>**、**<UI-TERM key="ocp-v2.widgets.Chart.CompareModal.DailyYoy">日同比</UI-TERM>**、**<UI-TERM key="ocp-v2.widgets.Chart.CompareModal.ZhouYoy">周同比</UI-TERM>**、**<UI-TERM key="ocp-v2.widgets.Chart.CompareModal.MonthOnYear">月同比</UI-TERM>** 4 种监控对比方式，帮您轻松发现数据规律。您也可以依据数据的特点和需求选择如折线图、饼图、表格等 7 种不同的图表类型，这样您就可以帮助更好地理解和分析数据。
