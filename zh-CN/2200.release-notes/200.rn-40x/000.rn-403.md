...
* 优化当用户无 **<UI-TERM key="ocp-v2.Detail.Monitor.Monitor.Overview">总览</UI-TERM>** 页面权限时，登录 OCP 后报错的问题。
...
* **<UI-TERM key="ocp-v2.Host.Detail.Surveillance">监控</UI-TERM>**

    **<UI-TERM key="ocp-v2.MetricChart.DrilldownDrawer.SubDrilldownDrawer.Tenant">租户</UI-TERM>** > **<UI-TERM key="ocp-v2.Detail.Monitor.Title.PerformanceMonitoring">性能监控</UI-TERM>** 中新增分布式相关监控项（仅支持 OceanBase V3.2.4-bp1 、V4.1 及以上版本）：

    * **性能与 SQL** 下新增 **SQL 执行时间** 监控图表，包含 `local`、`remote` 和 `distributed` 监控数据。

    * **<UI-TERM key="ocp-v2.Detail.Monitor.Transaction">事务</UI-TERM>** 下新增 **事务响应时间** 监控图表，包含 `local` 和 `distributed` 监控数据。

### 功能优化

* 优化平台 **<UI-TERM key="ocp-v2.Detail.Monitor.Monitor.Overview">总览</UI-TERM>** 页面权限，仅支持管理员用户查看平台总览信息。
...
* 调整 OCP 关于备份告警项的默认配置：
  ...
  * `base_backup_timeout`：告警等级由 **<UI-TERM key="ocp-v2.src.constant.alarm.Serious">严重</UI-TERM>** 调整为 **<UI-TERM key="ocp-v2.src.constant.alarm.Small">注意</UI-TERM>**。

  * `backup_storage_capacity_over_threshold`：告警等级由 **<UI-TERM key="ocp-v2.src.constant.alarm.Serious">严重</UI-TERM>** 调整为 **<UI-TERM key="ocp-v2.src.constant.task.Warning">警告</UI-TERM>**。

  * `backup_storage_capacity_timeout_or_interrupted`：告警等级由 **<UI-TERM key="ocp-v2.src.constant.alarm.Serious">严重</UI-TERM>** 调整为 **<UI-TERM key="ocp-v2.src.constant.task.Warning">警告</UI-TERM>**。
...
| 3  | 无 **<UI-TERM key="ocp-v2.Detail.Monitor.Monitor.Overview">总览</UI-TERM>** 页查看权限的用户登录 OCP 后，系统会默认定向到总览页且页面报错。      | 暂无。   |
