...
* （可选）已参考 [租户参数模板管理](../1500.tenant-parameter-template-management.md) 创建好参数模板。
...
5. 填写 **副本设置** 。
   ...
   * 当所选集群版本为 V4.0 以下时，多个 Zone 可以设置不同的 Unit 数量。
      ...
      | **Unit 规格**    | OCP 内置了一套 Unit 规格，请参考 [OCP 资源单元规格列表](../../1300.appendix-2/300.ocp-resource-unit-specifications.md) 进行选择，或者您也可以在列表下方单击 **新增规格** 按钮新增自定义规格。   <blockquote>**注意**</br>建议为全能型副本设置相同的 Unit 规格与数量，不同的 Unit 规格与数量可能造成性能或稳定性问题。</blockquote>  |
...
7. 打开 **参数配置** 模块，并配置租户参数。
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>在未创建租户参数模板的情况下，您可单击 <strong>新建租户模板</strong>，为租户创建参数模板，详情可参考 <a href="../1500.tenant-parameter-template-management.md">租户参数模板管理</a>。</p>
  </main>
