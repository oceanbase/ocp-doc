...
6. 单击 **确定** 。
   ...
   * 子任务：升级所有 OceanBase 集群版本。主要是执行升级脚本: 预检查脚本、升级 Pre 脚本、替换 OBServer 节点、升级 Post 脚本、版本检查等。
    ...
    <h4>注意</h4>
    <p>当 OceanBase 集群由 V4.0 版本升级至 V4.1 版本时，由于 OceanBase V4.0 版本与 V4.1 版本的备份文件不兼容，升级时会报错 <code>upgrade checker failed with 2 reasons : [still has archive destination, upgrade is not allowed temporarily] , [still has backup destination, upgrade is not allowed temporarily] </code> ，此时您需进行如下操作：<ul><li>在升级前停止所有日志备份任务及数据备份任务。</li><li>升级后，黑屏登录对应的操作租户，执行如下命令在备份策略中更换新的备份目录。</br><code>ALTER SYSTEM SET LOG_ARCHIVE_DEST='';</code></br><code>ALTER SYSTEM SET DATA_BACKUP_DEST='';</code> </p>
    </main>

7. 当任务状态为 **完成** ，且 **集群** 页的 **集群列表** 中该集群的状态均为 **运行中** 时，表示升级成功。
