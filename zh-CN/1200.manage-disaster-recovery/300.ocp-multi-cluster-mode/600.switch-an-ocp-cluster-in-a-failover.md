|slug|switch-an-ocp-cluster-in-a-failover|
|---|---|

# 容灾切换 OCP 集群（Failover）

本页面为您介绍将 OCP 备集群容灾切换为主集群的操作。当主集群不可连接时，可通过灾备切换，将备集群切换为主集群。执行 Failover 操作后，对应的备集群角色将变换成 PRIMARY，与原主集群不再存在关联关系。若您存在多集群模式需求，需重新搭建主备 OCP 集群。
...
4. 单击 **<UI-TERM key="ocp-v2.Detail.Component.FailoverDrawer.DisasterRecoverySwitch">容灾切换</UI-TERM>** 。
