...
| status      | StringArray | 否  | CREATING,RUNNING | 集群的状态。 取值范围： * RUNNING：运行中。   * CREATING：创建中。   * DELETING：删除中。   * STARTING：启动中。   * RESTARTING：重启中。   * STOPPING：停止中。   * STOPPED：已停止。   * TAKINGOVER：接管中。   * MOVINGOUT：迁出中。   * SWITCHOVER：主备集群切换中。   * FAILOVER：备集群故障恢复中。   * OPERATING 运维中。    |
...
| status                  | String   | 集群的状态。 取值范围： * RUNNING：运行中。   * CREATING：创建中。   * DELETING：删除中。   * STARTING：启动中。   * RESTARTING：重启中。   * STOPPING：停止中。   * STOPPED：已停止。   * TAKINGOVER：接管中。   * MOVINGOUT：迁出中。   * SWITCHOVER：主备集群切换中。   * FAILOVER：备集群故障恢复中。   * OPERATING：运维中。    |
