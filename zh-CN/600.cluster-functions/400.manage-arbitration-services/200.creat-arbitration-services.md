...
|   enable_syslog_recycle   |   是否开启 log 文件自动回收，默认值为 `False`   |   设为 `True` 时，可同时配置 `max_syslog_file_count` 指定要保留的日志文件数量。<main id="notice" type='notice'><h4>注意</h4>建议保持该参数为开启状态，否则可能导致日志盘被打满、从而仲裁服务不可用。</main>  |
