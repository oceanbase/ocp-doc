...
1. 确认是否 OceanBase 集群故障导致的无法同步的原因。
   ...
   此时，可先参照 [ob_cluster_status_check_failed OB 集群状态检测失败](2.failed-to-check-the-status-of-the-ob_cluster_status_check_failed-ob-cluster.md) 告警的处理方法排除故障，等待 5 分钟后，观察告警是否继续上报。
   * 是，则还有其他问题，请继续执行下一步。
