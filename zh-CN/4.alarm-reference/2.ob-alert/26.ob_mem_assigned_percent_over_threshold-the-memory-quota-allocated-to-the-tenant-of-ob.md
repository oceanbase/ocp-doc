...
1. 检查 OceanBase 集群中是否存在下线的 OBServer。
   ...
   * 存在，则会同时上报 [ob_cluster_exists_inactive_server OB 集群存在不工作 OBServer](/zh-CN/4.alarm-reference/2.ob-alert/3.ob_cluster_exists_inactive_server-ob-the-cluster-is-not-working.md) ，请参考文档先处理该告警，再观察本节所述告警是否还上报。
...
2. 可能是 OceanBase 集群的剩余内存不足。
   ...
   2. 对实际内存使用小于分配的租户进行缩容，即调整该租户的 Unit 规格，请参考 [Unit 规格管理](/zh-CN/3.ob-cloud-platform/5.manage-tenants/2.basic-tenant-operations/2.unit-specification-management.md) 。
