...
| 指标来源 |指标来源：`chronyc sources &#124; grep -e '^\\^\\*' &#124; sed 's/.*\\[\\(.*\\)\\].*/\\1/g' ntpq -np 192.168.0.1 &#124; grep -v '192.168.0.1' &#124;/bin/grep -e '^*' ntpdc -np 192.168.0.1 &#124;grep -v '192.168.0.1' &#124;grep -e '^*' ` </br>该告警的指标来源相对特殊，是由 OCP-Agent 使用时钟服务来获取时钟偏移数据，OCP-Agent 默认使用 chronyd 获取时钟偏移，采集命令为 `chronyc tracking -n`，取其中的 Last offset。如果主机上不存在 chronyd 而存在 ntpd，则使用 ntp 采集时钟偏移，需要在配置文件中指定 `monagent.node.ntp.server`，默认为 127.0.0.1。 |
