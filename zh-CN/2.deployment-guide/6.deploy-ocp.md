...
* 已参见 [安装准备](../2.deployment-guide/4.installation-preparation.md) 获取 ocp-3.1.1-ce.tar.gz 软件包。
  ...
  * 以 root 用户将软件包中的 OCP 镜像文件 `ocp-3.1.1-ce-bp1.tar.gz` 上传到宿主机的任一目录。

  * 执行tar -zxvf ocp-3.1.1-ce-bp1.tar.gz解压镜像文件。

操作步骤
-------------------------

1. 以 root 用户登录 OCP 的宿主机，并进入到解压后的镜像目录ocp-3.1.1-ce-bp1文件路径，找到ocp_installer.sh文件。
