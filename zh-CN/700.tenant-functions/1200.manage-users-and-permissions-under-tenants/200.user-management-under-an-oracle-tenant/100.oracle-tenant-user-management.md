...
用户是数据库中定义的可以连接和访问对象的名称。您可在 **<UI-TERM key="ocp-v2.Oracle.UserOrRoleDetail.UserList">用户列表</UI-TERM>** 页签内，查看 **用户名、拥有系统权限、可访问对象、拥有角色、新建时间** 和 **<UI-TERM key="ocp-v2.User.Oracle.UserList.LogonConnectionString">登录连接串</UI-TERM>** 等信息，同时对用户进行 **锁定、修改密码** 和 **<UI-TERM key="ocp-v2.Detail.Overview.Delete">删除</UI-TERM>** 等操作。
