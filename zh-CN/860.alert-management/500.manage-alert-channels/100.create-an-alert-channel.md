...
告警本身是独立的功能，未配置通道时，只能通过控制台的 **<UI-TERM key="ocp-v2.page.Alarm.AlarmEvent">告警事件</UI-TERM>** 页面查看告警。通过配置告警通道，用户可以接收告警通知消息。
...
4. 在告警通道页签中，单击 **<UI-TERM key="ocp-v2.Alarm.Channel.NewChannel">新建通道</UI-TERM>** 。
...
6. 当 **<UI-TERM key="ocp-v2.Alarm.Channel.New.ChannelType">通道类型</UI-TERM>** 选择为 **HTTP**、**<UI-TERM key="ocp-v2.Auth.User.New.Mailbox">邮箱</UI-TERM>** 或 **<UI-TERM key="ocp-v2.src.constant.alarm.CustomScript">自定义脚本</UI-TERM>** 时，需根据界面提示进行通道配置。
...
7. 设置 **<UI-TERM key="ocp-v2.Alarm.Channel.New.MessageConfiguration">消息配置</UI-TERM>** 。

   1. 告警消息格式支持 Markdown 和 Json 两种格式。您可根据实际所需，对告警消息模板及告警恢复消息模板进行个性化配置。配置完成后，系统所推送的消息将依照您所配置的模板样式进行展示。

   2. 在 **重复告警配置** 中，为相同告警设置发送次数和时间间隔，用于控制同一个告警是否重复发送告警消息以及告警消息的发送频率。其中，告警时间间隔默认为 600 秒，最小取值为 300 秒，所配置的值需大于或等于告警规则中的检测周期。

   3. 选择是否开启 **<UI-TERM key="ocp-v2.Alarm.Channel.Detail.AlertMessageAggregation">告警消息聚合</UI-TERM>** 功能。开启消息聚合，可以避免一段时间内告警发送过多。开启告警消息聚合之后，OCP 告警默认 5 分钟（从 V4.1.0 版本开始）产生一次告警。

8. 单击 **<UI-TERM key="ocp-v2.Alarm.Channel.New.SendTestMessages">发送测试消息</UI-TERM>** ，如果发送成功，可单击 **<UI-TERM key="ocp-v2.Inspection.Item.ConfigRule.Submit">提交</UI-TERM>** 。
