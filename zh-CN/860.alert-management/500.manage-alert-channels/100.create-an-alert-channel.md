...
7. 设置 **<UI-TERM key="ocp-v2.Alarm.Channel.New.MessageConfiguration">消息配置</UI-TERM>** 。
   ...
   同时，您可选择是否开启 **<UI-TERM key="ocp-v2.Alarm.Channel.Detail.AlertMessageAggregation">告警消息聚合</UI-TERM>** 功能。开启消息聚合，可以避免一段时间内一条告警发送多次。OCP 告警默认 3 分钟（从 V4.1.0 版本开始）产生一次告警，开启告警消息聚合之后，相同告警默认 600 秒仅发送一次。
